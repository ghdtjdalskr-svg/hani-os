...
`;
  ...
  function renderMain() {
    ...
    if (activeQuiz && activeQuiz.status !== 'completed') {
      qa('input[type="radio"]', q('#studyQuizDetail')).forEach(input => input.onchange = () => {
        ...
        activeQuiz.answers[qi] = value;
        activeQuiz.updatedAt = nowIso();
        try { save(); } catch (_) {}
        renderLearning();
      });
  ...
  function bindStaticEvents() {
    ...
    q('#studyProjectSave')?.addEventListener('click', () => {
      ...
      activeQuizId = '';
      if (!learningCommit(`학습 프로젝트 ‘${row.name}’을 시작했습니다.`)) return;
      renderLearning();
    });
  }
  ...
  async function generateTodayQuiz(project) {
    ...
    try {
      ...
      if (!learningCommit(`${project.name} 오늘의 퀴즈 5문제를 생성했습니다.`)) throw new Error('퀴즈 저장 검증에 실패했습니다.');
  ...
  function gradeQuiz(quiz) {
    ...
    if (!learningCommit(`${project.name} 퀴즈 채점 완료 · ${quiz.score}점`)) return;
    activeQuizId = quiz.id;
    renderLearning();
  }
  ...
  window.HANI_STUDY_V02984_TEST = { ensureLearningState, normalizeProject, normalizeQuestion, normalizeQuiz, questionKey, derivedLearningTasks, upsertWrongAnswer };
  ...
  function boot() {
    ...
    document.addEventListener('click', (ev) => {
      if (ev.target.closest?.('[data-view="study"]')) setTimeout(renderLearning, 0);
      if (ev.target.closest?.('[data-view="home"]')) setTimeout(renderHomeDerived, 0);
    }, true);
    window.HANI_STUDY_V02984_AUDIT = () => ({
      version: VERSION,
      storageKeyUnchanged: typeof STORAGE_KEY === 'undefined' ? null : STORAGE_KEY === 'hani_os_life_v23',
