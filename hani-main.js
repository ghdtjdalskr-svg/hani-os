...
function renderBody(){
  ...
  const active=state.ui.bodyMetric||'weight';$('bodyMetricTabs').innerHTML=[...Object.entries(BODY_METRICS),['summary',{label:'전체 요약'}]].map(([k,v])=>`<button class="tab ${active===k?'active':''}" data-body-metric="${k}">${v.label}</button>`).join('');document.querySelectorAll('[data-body-metric]').forEach(btn=>btn.onclick=()=>{state.ui.bodyMetric=btn.dataset.bodyMetric;save();renderBody()});
...
function movieWorkCard(group){const latest=group.rows[0]?.record||{},ratings=group.rows.map(x=>ratingValue(x.record.rating)).filter(x=>x!==null),avg=ratings.length?(ratings.reduce((a,b)=>a+b,0)/ratings.length).toFixed(1):'',multi=group.rows.length>1,preview=movieReviewBody(latest.review||'');return `<article class="media-work-row"><div class="media-work-identity">${latest.poster?`<img src="${esc(latest.poster)}" alt="${esc(group.title)} 포스터">`:'<span class="media-work-placeholder">🎬</span>'}<div><span>${esc(group.type)} · ${multi?group.rows.length+'개 기록':latest.status==='watched'?'관람 완료':'관람 예정'}</span><h4>${esc(group.title)}</h4><small>${esc(latest.watchedDate||'날짜 미입력')}${avg?` · ★ ${avg}`:''}</small></div></div><p>${esc(preview||'남긴 감상이나 기대평이 없습니다.')}</p>${multi?`<details><summary>시즌·회차 ${group.rows.length}개 보기</summary><div class="media-episode-stack">${group.rows.map(({record,meta})=>`<div class="media-episode-row"><span><b>${meta?`S${String(meta.season).padStart(2,'0')}${meta.episode?` · EP${String(meta.episode).padStart(2,'0')}`:''}`:esc(record.title)}</b><small>${esc(record.watchedDate||'날짜 미입력')}${record.rating?` · ★ ${esc(record.rating)}`:''}</small></span><p>${esc(movieReviewBody(record.review||'')||'코멘트 없음')}</p><div class="archive-actions"><button class="btn sm" data-movie-toggle="${record.id}">관람 예정작으로</button><button class="btn sm" data-movie-edit="${record.id}">수정</button><button class="btn sm danger" data-movie-delete="${record.id}">삭제</button></div></div>`).join('')}</div></details>`:`<div class="archive-actions"><button class="btn sm" data-movie-toggle="${latest.id}">${latest.status==='watched'?'관람 예정작으로':'관람 완료로'}</button><button class="btn sm" data-movie-edit="${latest.id}">수정</button><button class="btn sm danger" data-movie-delete="${latest.id}">삭제</button></div>`}</article>`}
function renderMovies(){
  ...
  $('movieCount').textContent=state.movies.length+'편';$('wishMovieCount').textContent=wish.length+'작품';$('watchedMovieCount').textContent=`${watched.length}작품 · ${watchedRecords.length}기록`;$('movieStats').innerHTML=[['전체 기록',state.movies.length+'편'],['관람 예정',allWish.length+'편'],['관람 완료',completedWatched.length+'편'],['평균 평점',avg?`${avg.toFixed(1)}점 · 만점 ${five}편`:'-']].map(([l,v])=>`<div class="stat"><div class="label">${l}</div><div class="value">${v}</div></div>`).join('');
...
function renderDiary(){
  ...
  $('diaryCount').textContent=all.length+'개';const pageData=uiPageSlice('diary',rows,uiPageSize(8,5));$('diaryList').innerHTML=pageData.rows.map(diaryCard).join('')||'<div class="empty">조건에 맞는 일기가 없습니다.</div>';uiMountPagination($('diaryList'),'diary',pageData,renderDiary,'개');$('diarySearch').value=search;$('diarySort').value=sort;$('diarySearch').oninput=()=>{state.ui.diarySearch=$('diarySearch').value;uiResetPage('diary');renderDiary()};$('diarySort').onchange=()=>{state.ui.diarySort=$('diarySort').value;uiResetPage('diary');save();renderDiary()};document.querySelectorAll('[data-diary-view]').forEach(b=>b.onclick=()=>openDiaryDetail(b.dataset.diaryView));document.querySelectorAll('[data-diary-edit]').forEach(b=>b.onclick=()=>{document.querySelector('#diary .phase1-collapsible-form')?.setAttribute('open','');editDiary(b.dataset.diaryEdit)});document.querySelectorAll('[data-diary-delete]').forEach(b=>b.onclick=()=>deleteDiary(b.dataset.diaryDelete));
}
...
const HANI_DISPLAY_VERSION="2.9.115";
