...
function renderTravelDetailLists(t){
  ...
  $("travelReviewCount").textContent=`${t.reviews.length}개`;
  $("travelReviewList").innerHTML=t.reviews.length?t.reviews.map(r=>`<div class="travel-review" data-travel-review="${esc(r.id)}"><div class="travel-review-head"><div><b>${travelArchiveIcon(r.type)} ${esc(r.name)}</b><div class="travel-review-meta">${travelReviewMetaHtml(r)}</div></div><button class="btn sm danger travel-review-delete" type="button">삭제</button></div>${r.review?`<div class="travel-review-body">${esc(r.review)}</div>`:""}</div>`).join(""):`<div class="empty">아직 연결된 맛집·명소 기록이 없어요.</div>`;
...
function renderTravelArchive(trips){
  const all=trips.flatMap(t=>(t.reviews||[]).map(r=>({trip:t,review:r}))),typeSel=$("travelArchiveType"),tripSel=$("travelArchiveTrip");if(!typeSel||!tripSel)return;
  ...
  tripSel.innerHTML='<option value="all">전체 여행</option>'+trips.map(t=>`<option value="${esc(t.id)}">${esc(t.name||t.destination||"이름 없는 여행")}</option>`).join("");
  typeSel.value=types.includes(wantedType)?wantedType:"all";tripSel.value=trips.some(t=>t.id===wantedTrip)?wantedTrip:"all";state.ui.travelArchiveType=typeSel.value;state.ui.travelArchiveTrip=tripSel.value;
  const rows=all.filter(x=>(typeSel.value==="all"||travelArchiveGroup(x.review.type)===typeSel.value)&&(tripSel.value==="all"||x.trip.id===tripSel.value)).sort((a,b)=>String(b.review.visitDate||b.review.createdAt||"").localeCompare(String(a.review.visitDate||a.review.createdAt||"")));
  $("travelArchiveCount").textContent=`${rows.length}개`;
  $("travelArchiveGrid").innerHTML=rows.length?rows.map(({trip:t,review:r})=>`<article class="travel-archive-card"><div class="travel-archive-card-top"><span class="travel-archive-type">${travelArchiveIcon(r.type)} ${esc(r.type||travelArchiveGroup(r.type))}</span><span class="travel-archive-trip">원본 여행 · ${esc(t.name||t.destination||"여행")}</span></div><h4>${esc(r.name||"이름 없는 기록")}</h4><div class="travel-archive-meta">${[r.visitDate,r.location,r.rating?`★ ${r.rating.toFixed(1)} / 5`:"",travelCostLabel(r.cost)].filter(Boolean).map(esc).join(" · ")}</div>${r.review?`<p>${esc(r.review)}</p>`:""}<div class="travel-archive-footer"><span>재방문 의견 · ${esc(travelReturnLabel(r.returnVisit))}</span><button class="btn sm travel-archive-open" data-trip-id="${esc(t.id)}" type="button">원본 여행 보기</button></div></article>`).join(""):`<div class="empty" style="grid-column:1/-1">조건에 맞는 맛집·명소 기록이 없어요. 여행 상세에서 기존 후기 기록을 추가해 보세요.</div>`;
  $("travelArchiveGrid").querySelectorAll(".travel-archive-open").forEach(btn=>btn.onclick=()=>openTravelTripDetail(btn.dataset.tripId));
  typeSel.onchange=()=>{state.ui.travelArchiveType=typeSel.value;renderTravel()};tripSel.onchange=()=>{state.ui.travelArchiveTrip=tripSel.value;renderTravel()};
}
function renderTravel(){
  ...
  const reviews=trips.flatMap(t=>t.reviews||[]),rated=reviews.filter(r=>n(r.rating)>0),avg=rated.length?rated.reduce((a,r)=>a+n(r.rating),0)/rated.length:0,destinations=new Set(trips.map(t=>String(t.destination||"").trim()).filter(Boolean)),nextWish=wish.find(w=>w.expectedDate&&w.expectedDate>=today())||wish[0];
  $("travelKpis").innerHTML=`<div class="module-kpi"><small>완료 여행</small><b>${trips.length}회</b><div class="sub">기존 여행 원본</div></div><div class="module-kpi"><small>기록한 지역</small><b>${destinations.size}곳</b><div class="sub">목적지 기준</div></div><div class="module-kpi"><small>후기 평균</small><b>${rated.length?avg.toFixed(1):"-"}</b><div class="sub">${reviews.length}개 장소 기록</div></div><div class="module-kpi"><small>다음 여행</small><b>${nextWish?esc(nextWish.destination):"아직 없음"}</b><div class="sub">${nextWish?.expectedDate?esc(nextWish.expectedDate):"자유 메모부터 시작해요"}</div></div>`;
  $("travelTripCount").textContent=`${trips.length}회`;
  $("travelTripGrid").innerHTML=trips.length?trips.map(t=>{const tRated=t.reviews.filter(r=>n(r.rating)>0),tAvg=tRated.length?tRated.reduce((a,r)=>a+n(r.rating),0)/tRated.length:0;return `<article class="travel-card" data-travel-trip="${esc(t.id)}"><span class="travel-destination">${esc(t.destination||"목적지 미등록")}</span><h4>${esc(t.name||t.destination||"이름 없는 여행")}</h4><div class="travel-card-meta"><span>${esc(travelDateRange(t))}</span>${t.companions?`<span>· 동행 ${esc(t.companions)}</span>`:""}</div><div class="travel-card-note">${esc(t.summary||"이번 여행의 한줄평을 남겨보세요.")}</div><div class="travel-plan-chips"><span class="travel-plan-chip">일정 ${t.itinerary.length}</span><span class="travel-plan-chip">후기 ${t.reviews.length}</span>${tRated.length?`<span class="travel-plan-chip warm">★ ${tAvg.toFixed(1)}</span>`:""}${t.sourceWishId?'<span class="travel-plan-chip">Wish 전환</span>':""}</div><div class="travel-card-actions"><button class="btn sm primary travel-trip-open" type="button">상세·일정 보기</button><button class="btn sm travel-trip-edit" type="button">수정</button></div></article>`}).join(""):`<div class="empty" style="grid-column:1/-1">완료 여행 기록이 없어요. 여행 기본정보부터 가볍게 등록해 보세요.</div>`;
...
const HANI_DISPLAY_VERSION="2.9.129";
