...
const HANI_LOGO_SYSTEM_VERSION="2.9.114";
const HANI_CANONICAL_LOGO_OWNERS=Object.freeze({
  brand:"hani-main",
  investment:"hani-main",
  newsroom:"hani-ui-v02983"
});
...
function normalizeTravelPlace(x={}){const base=normalizeTravelReview(x);return {...base,highlight:String(x.highlight||"").trim(),priceRange:String(x.priceRange||"").trim(),tripId:String(x.tripId||"").trim(),sourceStopId:String(x.sourceStopId||"").trim()}}
...
function normalizeState(d){
  ...
  const normalized={
    ...
    travelPlaces:Array.isArray(d?.travelPlaces)?d.travelPlaces.map(normalizeTravelPlace):[],
...
const HANI_BRAND_LOGO_ASSET="./assets/brand/hani-group-ci-official.webp";
const INVESTMENT_LOGO_COMPANIES=[
  ...
  {id:"lg",rx:/(LG전자(?:우)?|066570|066575|LG ELECTRONICS)/i,label:"LG Electronics",color:"#a50034",fallback:"LG",icon:"https://cdn.simpleicons.org/lg/FFFFFF"},
  {id:"nvidia",rx:/(엔비디아|NVIDIA|\bNVDA\b)/i,label:"NVIDIA",color:"#76b900",fallback:"N",icon:"https://cdn.simpleicons.org/nvidia/FFFFFF"},
  {id:"amazon",rx:/(아마존|AMAZON|\bAMZN\b|\bAWS\b)/i,label:"Amazon",color:"#232f3e",fallback:"a",kind:"amazon"},
...
const INVESTMENT_LOGO_ISSUERS=[
  ...
  {id:"tiger",rx:/\bTIGER\b/i,label:"TIGER",color:"#e34a3f"},
  {id:"robo",rx:/\bROBO\b|로보\s*글로벌|로보틱스\/자동화/i,label:"ROBO",color:"#2877ef"},
  {id:"rise",rx:/\bRISE\b/i,label:"RISE",color:"#e4b900",dark:true},
  {id:"plus",rx:/\bPLUS\b/i,label:"PLUS",color:"#f47a1f"},
  {id:"koact",rx:/\bKOACT\b/i,label:"KOACT",color:"#2468f2"}
];
const INVESTMENT_LOGO_FALLBACK_COLORS=["#61748d","#4b78a8","#4b8e82","#8a6da8","#a06b54","#6675a8"];
function investmentLogoTheme(raw=""){
  const s=String(raw).toUpperCase();
  if(/NASDAQ\s*100|나스닥\s*100|나스닥100/.test(s))return{kind:"text",label:"N100"};
  ...
  if(/ROBO|로봇|자동화|ROBOT|휴머노이드/.test(s))return{kind:"robot"};
  if(/방산|DEFEN/.test(s))return{kind:"shield"};
  if(/헬스|HEALTH/.test(s))return{kind:"health"};
  if(/AI|인공지능|전력|인프라/.test(s))return{kind:"ai"};
  if(/금액티브|GOLD|골드|금\s/.test(s))return{kind:"gold"};
  if(/커버드콜|COVERED/.test(s))return{kind:"covered"};
  return{kind:"etf"};
}
function investmentLogoStableKey(raw=""){
  let hash=0;for(const c of String(raw))hash=(hash*31+c.codePointAt(0))>>>0;return hash;
}
function investmentLogoFallbackMark(name="",ticker=""){
  const latin=String(ticker||"").toUpperCase().replace(/[^A-Z0-9]/g,"");
  if(latin&&!/^\d+$/.test(latin))return latin.slice(0,4);
  const cleaned=String(name||"").replace(/\([^)]*\)/g," ").replace(/[^0-9A-Za-z가-힣]+/g," ").trim();
  const words=cleaned.split(/\s+/).filter(Boolean);
  if(words.length>1&&words.some(w=>/[A-Za-z]/.test(w)))return words.map(w=>w[0]).join("").toUpperCase().slice(0,4);
  return (cleaned.match(/[가-힣]/g)||cleaned.match(/[A-Za-z0-9]/g)||["·"]).slice(0,3).join("").toUpperCase();
}
function resolveInvestmentLogo(input={}){
  const h=typeof input==="string"?{name:input}:input||{},m=investmentSecurityMeta(h);
  const name=String(h.name||"종목").trim(),ticker=String(h.ticker||m.ticker||"").trim(),raw=[name,ticker,h.issuer||h.issuerName||h.provider||h.fundHouse||"",h.className||"",m.typeLabel||""].join(" ");
  const company=INVESTMENT_LOGO_COMPANIES.find(x=>x.rx.test(raw));
  if(company)return{...company,type:"stock",source:company.icon?"mapped":"fallback"};
  const issuer=INVESTMENT_LOGO_ISSUERS.find(x=>x.rx.test(raw));
  if(issuer)return{...issuer,type:"etf",fallback:issuer.label,theme:investmentLogoTheme(raw),source:"fallback"};
  const key=investmentLogoStableKey(`${name}|${ticker}|${m.typeKey}`),isEtf=m.typeKey==="etf"||/\bETF\b/i.test(raw);
  return{id:`fallback-${key.toString(36)}`,type:isEtf?"etf":m.typeKey||"other",label:name||ticker||"종목",fallback:investmentLogoFallbackMark(name,ticker),theme:isEtf?investmentLogoTheme(raw):null,color:INVESTMENT_LOGO_FALLBACK_COLORS[key%INVESTMENT_LOGO_FALLBACK_COLORS.length],source:"fallback"};
}
function investmentLogoIconSvg(kind="etf"){
  ...
  const icons={
    ...
    dividend:`<svg ${c}><circle cx="12" cy="12" r="8"/><path d="M8.5 12h7M12 8.5v7"/></svg>`,
    robot:`<svg ${c}><rect x="5" y="7" width="14" height="11" rx="3"/><path d="M9 12h.01M15 12h.01M9 16h6M12 3v4"/></svg>`,
    shield:`<svg ${c}><path d="M12 3 19 6v5c0 4.6-2.8 8-7 10-4.2-2-7-5.4-7-10V6l7-3Z"/></svg>`,
    ...
    covered:`<svg ${c}><path d="M4 8h11l-3-3M20 16H9l3 3"/></svg>`,
    etf:`<svg ${c}><path d="M4 18V9M10 18V5M16 18v-7M22 18V3"/></svg>`
  };
  return icons[kind]||icons.etf;
}
function investmentSecurityLogoHtml(h={}){
  const identity=resolveInvestmentLogo(h),theme=identity.theme||{};
  const glyph=identity.type==="etf"?(theme.kind==="text"?`<b class="hani-investment-logo-theme">${esc(theme.label)}</b>`:investmentLogoIconSvg(theme.kind)):`<b class="hani-investment-logo-fallback">${esc(identity.fallback)}</b>`;
  const issuer=identity.type==="etf"?`<small>${esc(identity.fallback)}</small>`:"";
  const image=identity.icon?`<img src="${esc(identity.icon)}" alt="" loading="eager" referrerpolicy="no-referrer" onload="this.parentElement.classList.add('is-loaded')" onerror="this.remove()">`:"";
  return `<span class="hani-investment-logo type-${esc(identity.type)} brand-${esc(identity.id)}${identity.dark?" dark":""}" data-hani-investment-logo="${esc(identity.id)}" data-logo-source="${esc(identity.source)}" role="img" aria-label="${esc(identity.label)} 로고" style="--investment-logo-color:${esc(identity.color)}"><span class="hani-investment-logo-glyph">${glyph}${issuer}</span>${image}</span>`;
}
function investmentSecurityIdentityHtml(h={},detailHtml=""){
  return `<div class="hani-investment-security">${investmentSecurityLogoHtml(h)}<div class="hani-investment-security-copy"><b class="security-name">${esc(h.name||"종목")}</b>${detailHtml}</div></div>`;
}
function investmentSecurityNameHtml(h={}){const m=investmentSecurityMeta(h);return investmentSecurityIdentityHtml(h,`<div class="security-meta"><span class="security-badge type-${m.typeKey}">${esc(m.typeLabel)}</span><span class="security-badge market-${m.marketKey}">${m.marketText}</span>${m.ticker?`<span class="security-ticker">${esc(m.ticker)}</span>`:""}</div>`)}
function normalizeHaniBrandLogos(root=document){
  root.querySelectorAll?.('[data-hani-brand-logo]').forEach(host=>{
    const logos=Array.from(host.querySelectorAll(':scope > img'));
    const logo=logos.shift()||document.createElement('img');
    logos.forEach(x=>x.remove());
    logo.src=HANI_BRAND_LOGO_ASSET;logo.alt="HANI GROUP 공식 로고";logo.width=1774;logo.height=887;
    if(!logo.parentElement)host.appendChild(logo);
  });
}
window.HANI_LOGO_SYSTEM=Object.freeze({version:HANI_LOGO_SYSTEM_VERSION,owners:HANI_CANONICAL_LOGO_OWNERS,brandAsset:HANI_BRAND_LOGO_ASSET,resolveInvestment:resolveInvestmentLogo});
...
function brokerDisplayHoldings(rows=[]){const display=[],completeness=h=>[h?.quantity,h?.buyPrice,h?.currentPrice,h?.purchaseAmount,h?.evaluationAmount,h?.pnl,h?.returnRate].filter(v=>v!==null&&v!==undefined&&v!=="").length;for(const h of rows){if(!String(h?.name||h?.ticker||"").trim())continue;const name=String(h.name||"").trim().replace(/\s+/g," ").toLocaleLowerCase("ko-KR"),ticker=String(h.ticker||"").trim().toUpperCase().replace(/^A(?=\d{6}$)/,""),index=display.findIndex(old=>{const oldName=String(old.name||"").trim().replace(/\s+/g," ").toLocaleLowerCase("ko-KR"),oldTicker=String(old.ticker||"").trim().toUpperCase().replace(/^A(?=\d{6}$)/,"");return (!ticker||!oldTicker||ticker===oldTicker)&&((name&&name===oldName)||(!name&&brokerHoldingKey(old)===brokerHoldingKey(h)))});if(index<0)display.push(h);else if(completeness(h)>completeness(display[index]))display[index]=h}return display}
function brokerAggregatedHoldings(snapshot){
  const map=new Map();
  (snapshot?.accounts||[]).filter(a=>a.enabled).forEach(a=>brokerDisplayHoldings(a.holdings||[]).forEach(h=>{
...
function brokerHoldingHtml(aidx,hidx,h){
  ...
  const security=master||h,logo=String(security?.name||security?.ticker||"").trim()?investmentSecurityLogoHtml(security):"";
  return `<div class="broker-holding" data-broker-holding="${aidx}:${hidx}">
  <div class="rc2-holding-grid rc21">
    <div class="field holding-name"><label>종목 선택</label><div class="broker-security-select">${logo}<div><select data-bh="instrumentId">${brokerInstrumentOptions(selected)}</select><div class="sub master-meta">${master?`${esc(marketLabel(master.market))}${master.ticker?` · ${esc(master.ticker)}`:""}`:"종목 관리에서 먼저 등록하세요."}</div></div></div></div>
...
function investmentNewsArchivePosts(type=""){const rows=agentArray(investmentNewsArchiveRuntime.posts);return (type?rows.filter(x=>x.post_type===type&&!(type==="WEEKLY"&&agentObj(agentObj(x.payload).company_followup).version===1)):rows).sort((a,b)=>String(b.published_at||b.created_at||"").localeCompare(String(a.published_at||a.created_at||"")))}
...
function investmentNewsDisplayName(entity={}){const name=String(entity.name||'종목');return /^LG전자우(?:$|s)/.test(name)?'LG전자':name}
function investmentNewsFiveDayBrief(entity={}){
  const sorted=agentArray(entity.news).slice().sort((a,b)=>String(b.published_at||'').localeCompare(String(a.published_at||''))),cutoff=Date.now()-5*86400000;
  const rows=sorted.filter(x=>{const t=new Date(x.published_at||'').getTime();return Number.isFinite(t)&&t>=cutoff});
  const positive=rows.filter(x=>x.sentiment==='POSITIVE').length,negative=rows.filter(x=>x.sentiment==='NEGATIVE').length,neutral=Math.max(0,rows.length-positive-negative);
  const assessment=positive&&negative?'혼재':positive?'호재':negative?'악재':'중립',latest=rows[0]?.published_at||'';
  const points=[];rows.forEach(x=>{const point=String(x.summary||x.title||'').trim();if(point&&!points.includes(point)&&points.length<3)points.push(point)});
  const summary=points.length?`${points.join(' · ')} · 호재 ${positive} · 중립 ${neutral} · 악재 ${negative}`:'최근 5일 신규 뉴스 없음';
  return {assessment,latest,summary,count:rows.length,positive,neutral,negative}
}
function renderInvestmentNews(){
  if(!$("investmentNewsFeed"))return;
  window.HANI_COMPANY_FOLLOWUP_RENDER?.(investmentNewsArchiveRuntime.posts,investmentNewsArchiveRuntime.readSet,investmentNewsArchiveRuntime.loading,investmentNewsArchiveRuntime.error,id=>investmentNewsArchiveMarkRead({_archive_post_id:id}));
  ...
  if(targetBox){
    ...
    targetBox.querySelectorAll("[data-news-target]").forEach(btn=>btn.onclick=()=>{
      state.ui.investmentNewsEntity=btn.dataset.newsTarget||"all";uiResetPage('investmentNews');
  ...
  if($("investmentNewsEntityBriefs")){
    $("investmentNewsEntityBriefs").innerHTML=entities.length?entities.map(e=>{
      ...
      const brief=investmentNewsFiveDayBrief(e),displayName=investmentNewsDisplayName(e);return `<button type="button" class="investment-news-entity-row ${investmentNewsTone(e.signal)} ${isActive?"active":""}" data-news-entity="${esc(key)}"><span class="news-entity-logo">${investmentSecurityLogoHtml({...e,name:displayName})}</span><div class="news-entity-main"><b>${esc(displayName)}</b><span>최근 5일 · ${esc(brief.assessment)}${brief.latest?` · ${esc(String(brief.latest).slice(0,10))}`:""}</span></div><div class="news-entity-summary">${esc(brief.summary)}</div><span class="news-entity-signal ${investmentNewsTone(e.signal)}">${investmentNewsImpactLabel(e.signal)}</span></button>`
    ...
    $("investmentNewsEntityBriefs").querySelectorAll("[data-news-entity]").forEach(btn=>btn.onclick=()=>{
      ...
      state.ui.investmentNewsEntity=state.ui.investmentNewsEntity===key?"all":key;uiResetPage('investmentNews');
      save();
      renderInvestmentNews()
    })
  }
  ...
  const newsPage=uiPageSlice('investmentNews',rows,uiPageSize(8,5)),pageRows=newsPage.rows;
        ...
        ${pageRows.map((r,i)=>{
          ...
          return `<div class="investment-news-board-row newsroom-v03-row ${tone}" role="row">
            <button type="button" class="investment-news-row-main newsroom-v03-main" data-news-row-toggle="${i}" aria-expanded="false">
              <span class="news-col-symbol newsroom-v03-symbol">${investmentSecurityLogoHtml({...r.entity,name:investmentNewsDisplayName(r.entity)})}<span><b>${esc(investmentNewsDisplayName(r.entity))}</b><small>${investmentNewsFmtTime(r.item.published_at)}</small></span></span>
  ...
  uiMountPagination($("investmentNewsFeed"),'investmentNews',newsPage,renderInvestmentNews,'건');
  ...
  $("investmentNewsFeed").querySelectorAll("[data-news-journal]").forEach(btn=>btn.onclick=e=>{
    e.stopPropagation();
    const row=pageRows[Number(btn.dataset.newsJournal)];
...
function renderInvestmentHighlights(){
  ...
  const items=[
    {ico:"🏆",label:"이달의 효자종목",value:positive?positive.name:"아직 없음",sub:positive?signedWon(positive.pnl):"플러스 손익 종목 없음",security:positive},
    {ico:"🚀",label:"수익률 1위",value:bestRate?bestRate.name:"기록 없음",sub:bestRate?pct(n(bestRate.rate)):"-",security:bestRate},
    {ico:"🌱",label:"가장 많이 모은 종목",value:collector?collector.h.name:(prev?"추가매수 없음":"다음 기록부터"),sub:collector?`전월 대비 +${num(collector.d)}주`:(prev?"수량 증가 없음":"전월 비교 필요"),security:collector?.h},
    {ico:"👑",label:"이달의 일등계좌",value:bestAcc?bestAcc.a.accountName:"기록 없음",sub:bestAcc?pct(n(bestAcc.rate)):"-"},
    {ico:"💰",label:"가장 큰 계좌",value:largest?.accountName||"기록 없음",sub:largest?won(largest.assets):"-"}
  ];
  box.innerHTML=items.map(item=>`<div class="highlight-card">${item.security?investmentSecurityLogoHtml(item.security):`<span class="highlight-ico">${item.ico}</span>`}<div><small>${item.label}</small><b>${esc(String(item.value))}</b><span>${esc(String(item.sub))}</span></div></div>`).join("")
}
...
function renderAssets(){const c=calculate(),latest=officialBrokerLatest(),record=latest?brokerCalc(latest):null;if(record){$("assetStats").innerHTML=[["투자자산",won(record.total)],["평가금액",won(record.evaluation)],["총자산−평가액 차이",won(record.cashLike)],["평가손익",signedWon(record.pnl)]].map(([l,v])=>`<div class="stat"><div class="label">${l}</div><div class="value">${v}</div></div>`).join("");const total=record.total||1,residuals=record.accounts.filter(a=>Math.round(n(a.cashLike))!==0);$("assetComposition").innerHTML=[["미분류 차액",record.cashLike],["투자상품 평가액",record.evaluation]].map(([l,v])=>`<div class="item"><div class="row"><b>${l}</b><b>${won(v)}</b></div><div class="progress"><i style="width:${Math.max(0,Math.min(100,n(v)/total*100))}%"></i></div><div class="sub">${(n(v)/total*100).toFixed(1)}%</div></div>`).join("")+`<div class="archive-disclaimer"><b>계산 근거</b> · 총자산 ${won(record.total)} − 명시된 평가금액 ${won(record.evaluation)} = ${won(record.cashLike)}<br>현금으로 직접 확인된 금액이 아니라, 평가금액 누락분과 예수금 등이 함께 포함될 수 있는 미분류 차액입니다.${residuals.length?`<br>${residuals.map(a=>`${esc(a.accountName)} ${won(a.assets)} − ${won(a.evaluation)} = ${won(a.cashLike)}`).join("<br>")}`:""}<br>${esc(latest.period)} 월간 투자 기록 기준</div>`}else{$("assetStats").innerHTML=[["총자산",won(c.total)],["투자상품",won(c.totalMarket)],["현금",won(c.totalCash)],["평가손익",won(c.pnl)]].map(([l,v])=>`<div class="stat"><div class="label">${l}</div><div class="value">${v}</div></div>`).join("");const total=c.total||1;$("assetComposition").innerHTML=[["현금",c.totalCash],["투자상품",c.totalMarket]].map(([l,v])=>`<div class="item"><div class="row"><b>${l}</b><b>${won(v)}</b></div><div class="progress"><i style="width:${Math.max(0,Math.min(100,v/total*100))}%"></i></div><div class="sub">${(v/total*100).toFixed(1)}%</div></div>`).join("")}}
...
const uiDisplayPages=Object.create(null);
function uiPageSize(desktop=8,mobile=4){return window.matchMedia('(max-width:760px)').matches?mobile:desktop}
function uiPageSlice(key,rows,size){const total=rows.length,pages=Math.max(1,Math.ceil(total/size)),page=Math.min(pages,Math.max(1,Number(uiDisplayPages[key])||1));uiDisplayPages[key]=page;return {rows:rows.slice((page-1)*size,page*size),page,pages,total,size}}
function uiResetPage(key){uiDisplayPages[key]=1}
function uiMountPagination(anchor,key,pageData,render,label='기록'){
  if(!anchor)return;const base=anchor.tagName==='TBODY'?(anchor.closest('.tw')||anchor.closest('table')):anchor,id=anchor.id+'Pagination';let host=document.getElementById(id);
  if(!host){host=document.createElement('nav');host.id=id;host.className='hani-pagination';host.setAttribute('aria-label',label+' 페이지');base.after(host)}
  const {page,pages,total,size}=pageData;if(total<=size){host.hidden=true;host.innerHTML='';return}host.hidden=false;
...
function renderBody(){
  ...
  const ordered=[...rows].reverse(),pageData=uiPageSlice('body',ordered,uiPageSize(8,5));$('bodyCount').textContent=`${rows.length}건 · 전체 ${allRows.length}건`;$('bodyRows').innerHTML=pageData.rows.map(r=>`<tr><td>${esc(r.date)}</td><td>${bodyMetric(r.weight,'kg')}</td><td>${bodyMetric(r.fat,'%')}</td><td>${bodyMetric(r.muscle,'kg')}</td><td>${bodyMetric(r.bmi)}</td><td>${bodyMetric(r.fatMass,'kg')}</td><td><button class="btn sm danger" data-delete-body="${r.id}">삭제</button></td></tr>`).join('')||'<tr><td colspan="7">선택한 기간에 신체 기록이 없습니다.</td></tr>';uiMountPagination($('bodyRows'),'body',pageData,renderBody,'건');
...
function renderExercise(){
  const allRows=[...(state.exercise||[])].sort((a,b)=>String(a.date||'').localeCompare(String(b.date||''))),period=state.ui.exercisePeriod||'year',rows=exercisePeriodRows(allRows),totalSteps=rows.reduce((s,r)=>s+n(r.steps),0),totalDistance=rows.reduce((s,r)=>s+n(r.distance),0),strengthDays=rows.filter(r=>r.strength).length,stepDays=rows.filter(r=>n(r.steps)>0),avgSteps=stepDays.length?Math.round(totalSteps/stepDays.length):0;
  if($('exercisePeriod'))$('exercisePeriod').value=period;if($('exerciseSummaryPeriod'))$('exerciseSummaryPeriod').textContent=`${lifePeriodLabel(period)} 기준`;if($('exercisePeriodBasis'))$('exercisePeriodBasis').textContent=`${lifePeriodBasis(period)} · 전체 기록 ${allRows.length}건`;
  $('exerciseStats').innerHTML=[['기록 일수',rows.length+'일'],['총 걸음',totalSteps.toLocaleString()+'보'],['하루 평균 걸음',avgSteps.toLocaleString()+'보'],['총 거리',totalDistance.toFixed(1)+'km'],['근력운동',strengthDays+'일']].map(([l,v])=>`<div class="stat"><div class="label">${l}</div><div class="value">${v}</div></div>`).join('');
  const ordered=[...rows].reverse(),pageData=uiPageSlice('exercise',ordered,uiPageSize(8,5));$('exerciseCount').textContent=`${rows.length}건 · 전체 ${allRows.length}건`;$('exerciseRows').innerHTML=pageData.rows.map(r=>`<tr><td>${esc(r.date)}</td><td>${n(r.steps).toLocaleString()}보</td><td>${n(r.distance).toFixed(2)}km</td><td>${r.strength?'✅ 했음':'—'}</td><td>${esc(r.note||'')||'—'}</td><td><button class="btn sm danger" data-delete-exercise="${r.id}">삭제</button></td></tr>`).join('')||'<tr><td colspan="6">선택한 기간에 운동 기록이 없습니다.</td></tr>';uiMountPagination($('exerciseRows'),'exercise',pageData,renderExercise,'건');
  document.querySelectorAll('[data-delete-exercise]').forEach(b=>b.onclick=()=>{state.exercise=state.exercise.filter(r=>r.id!==b.dataset.deleteExercise);commit('운동 기록을 삭제했습니다.')});if($('exercisePeriod'))$('exercisePeriod').onchange=()=>{state.ui.exercisePeriod=$('exercisePeriod').value;uiResetPage('exercise');save();renderExercise()};drawExercise();
}
...
function renderReading(){
  const sort=state.ui.readingSort||'recent',min=Math.max(0,Math.min(5,n(state.ui.readingMinRating))),search=state.ui.readingSearch||'',allWish=state.books.filter(b=>b.status!=='read'),allRead=state.books.filter(b=>b.status==='read'),wish=sortRatedItems(allWish,'recent','completedDate'),read=sortRatedItems(filterMinimumRating(filterText(allRead,search,['title','author','topic','review']),min),sort,'completedDate'),completedRead=allRead.filter(b=>completedByToday(b.completedDate)),ratings=completedRead.map(b=>ratingValue(b.rating)).filter(v=>v!==null),avg=ratings.length?ratings.reduce((a,b)=>a+b,0)/ratings.length:0,five=ratings.filter(v=>v===5).length,currentYear=today().slice(0,4),month=today().slice(0,7);
  $('bookCount').textContent=state.books.length+'권';$('wishBookCount').textContent=wish.length+'권';$('readBookCount').textContent=read.length+'권';$('bookStats').innerHTML=[['전체 기록',state.books.length+'권'],['올해 완독',completedRead.filter(b=>completedInYearByToday(b.completedDate,currentYear)).length+'권'],['이번 달 완독',completedRead.filter(b=>completedInMonthByToday(b.completedDate,month)).length+'권'],['평균 평점',avg?`${avg.toFixed(1)}점 · 만점 ${five}권`:'-']].map(([l,v])=>`<div class="stat"><div class="label">${l}</div><div class="value">${v}</div></div>`).join('');
  const size=uiPageSize(12,4),readPage=uiPageSlice('readingRead',read,size),wishPage=uiPageSlice('readingWish',wish,size);$('wishBooks').innerHTML=wishPage.rows.map(bookCard).join('')||'<div class="empty">읽기 목록이 비어 있습니다.</div>';$('readBooks').innerHTML=readPage.rows.map(bookCard).join('')||'<div class="empty">조건에 맞는 서재 도서가 없습니다.</div>';uiMountPagination($('readBooks'),'readingRead',readPage,renderReading,'권');uiMountPagination($('wishBooks'),'readingWish',wishPage,renderReading,'권');
  $('bookSort').value=sort;$('bookMinRating').value=min||'';$('bookSearch').value=search;$('bookSort').onchange=()=>{state.ui.readingSort=$('bookSort').value;uiResetPage('readingRead');save();renderReading()};$('bookMinRating').oninput=()=>{state.ui.readingMinRating=Math.max(0,Math.min(5,n($('bookMinRating').value)));uiResetPage('readingRead');save();renderReading()};$('bookSearch').oninput=()=>{state.ui.readingSearch=$('bookSearch').value;uiResetPage('readingRead');renderReading()};updateBookFormState();renderReadingGoals(allRead);
  document.querySelectorAll('[data-book-delete]').forEach(b=>b.onclick=()=>{if(!confirm('이 책 기록을 삭제할까요?'))return;state.books=state.books.filter(x=>x.id!==b.dataset.bookDelete);commit('책을 삭제했습니다.')});document.querySelectorAll('[data-book-toggle]').forEach(b=>b.onclick=()=>{const x=state.books.find(v=>v.id===b.dataset.bookToggle);if(!x)return;x.status=x.status==='read'?'wish':'read';if(x.status==='read'&&!x.completedDate)x.completedDate=today();x.updatedAt=new Date().toISOString();commit('책 분류를 변경했습니다.')});document.querySelectorAll('[data-book-edit]').forEach(b=>b.onclick=()=>{const x=state.books.find(v=>v.id===b.dataset.bookEdit);if(!x)return;$('bookEditId').value=x.id;$('bookStatus').value=x.status||'wish';$('bookTitle').value=x.title||'';$('bookAuthor').value=x.author||'';$('bookTopic').value=x.topic||'교양';$('bookRating').value=x.rating??'';$('bookDate').value=x.completedDate||'';$('bookReview').value=x.review||'';$('saveBook').textContent='수정 저장';updateBookFormState();document.querySelector('#reading .phase1-collapsible-form')?.setAttribute('open','');$('reading').scrollIntoView({behavior:'smooth'})});
}
...
function movieReviewBody(value){return String(value||"").replace(/^\s*(?:시즌|season|s)\s*0*\d+(?:\s*[·|/]\s*(?:에피소드|episode|ep\.?|e|제)?\s*0*\d+\s*(?:화|까지)?)?\s*/i,"").trim()}
...
function updateMovieFormState(){const watched=$("movieStatus").value==="watched",series=["시리즈","드라마","애니메이션"].includes($("movieContentType").value);$("movieRating").disabled=!watched;$("movieDate").disabled=!watched;$("movieRatingField").classList.toggle("is-disabled",!watched);$("movieDateField").classList.toggle("is-disabled",!watched);["movieSeasonField","movieEpisodeField"].forEach(id=>$(id)?.classList.toggle("hidden",!series));$("movieReviewLabel").textContent=watched?"관람 후기 / Episode 코멘트":"보고 싶은 이유 / 기대평";$("movieReview").placeholder=watched?"관람 후 인상과 Episode 코멘트를 남겨요.":"이 작품을 보고 싶은 이유를 남겨요.";ratingPreview("movieRating","movieRatingPreview","관람 완료 후 입력할 수 있어요.")}
function resetMovieForm(){["movieEditId","movieTitle","movieDirector","movieActors","movieReview","movieSeason","movieEpisode"].forEach(id=>$(id).value="");$("movieStatus").value="wish";$("movieContentType").value="영화";$("movieOrigin").value="국내";$("movieRating").value="";$("movieDate").value="";$("moviePoster").value="";$("saveMovie").textContent="작품 저장";updateMovieFormState()}
function movieWorkGroups(records=[]){
  const groups=new Map();for(const record of records){const type=movieContentType(record.contentType,'기타'),meta=['시리즈','드라마','애니메이션'].includes(type)?movieSeriesMeta(record):null,key=meta?`series|${meta.base.toLocaleLowerCase()}`:`record|${record.id}`;if(!groups.has(key))groups.set(key,{key,title:meta?.base||record.title,type,rows:[]});groups.get(key).rows.push({record,meta})}
  return [...groups.values()].map(group=>{group.rows.sort((a,b)=>String(b.record.watchedDate||b.record.updatedAt||'').localeCompare(String(a.record.watchedDate||a.record.updatedAt||'')));return group}).sort((a,b)=>String(b.rows[0]?.record.watchedDate||b.rows[0]?.record.updatedAt||'').localeCompare(String(a.rows[0]?.record.watchedDate||a.rows[0]?.record.updatedAt||'')));
}
function movieViewingUnits(records=[]){return records.reduce((sum,record)=>sum+Math.max(1,movieSeriesMeta(record)?.episodes?.length||0),0)}
function movieWorkCard(group){const latest=group.rows[0]?.record||{},ratings=group.rows.map(x=>ratingValue(x.record.rating)).filter(x=>x!==null),avg=ratings.length?(ratings.reduce((a,b)=>a+b,0)/ratings.length).toFixed(1):'',units=movieViewingUnits(group.rows.map(x=>x.record)),multi=group.rows.length>1||units>1,preview=movieReviewBody(latest.review||'');return `<article class="media-work-row"><div class="media-work-identity">${latest.poster?`<img src="${esc(latest.poster)}" alt="${esc(group.title)} 포스터">`:'<span class="media-work-placeholder">🎬</span>'}<div><span>${esc(group.type)} · ${multi?units+'편 시청':latest.status==='watched'?'관람 완료':'관람 예정'}</span><h4>${esc(group.title)}</h4><small>${esc(latest.watchedDate||'날짜 미입력')}${avg?` · ★ ${avg}`:''}</small></div></div><p>${esc(preview||'남긴 감상이나 기대평이 없습니다.')}</p>${multi?`<details><summary>시즌·회차 ${units}편 보기</summary><div class="media-episode-stack">${group.rows.map(({record,meta})=>`<div class="media-episode-row"><span><b>${meta?`S${String(meta.season).padStart(2,'0')}${meta.episodes?.length?` · ${meta.episodes.map(value=>`EP${String(value).padStart(2,'0')}`).join(' · ')}`:''}`:esc(record.title)}</b><small>${esc(record.watchedDate||'날짜 미입력')}${record.rating?` · ★ ${esc(record.rating)}`:''}</small></span><p>${esc(movieReviewBody(record.review||'')||'코멘트 없음')}</p><div class="archive-actions"><button class="btn sm" data-movie-toggle="${record.id}">관람 예정작으로</button><button class="btn sm" data-movie-edit="${record.id}">수정</button><button class="btn sm danger" data-movie-delete="${record.id}">삭제</button></div></div>`).join('')}</div></details>`:`<div class="archive-actions"><button class="btn sm" data-movie-toggle="${latest.id}">${latest.status==='watched'?'관람 예정작으로':'관람 완료로'}</button><button class="btn sm" data-movie-edit="${latest.id}">수정</button><button class="btn sm danger" data-movie-delete="${latest.id}">삭제</button></div>`}</article>`}
function renderMovies(){
  const sort=state.ui.movieSort||'recent',min=Math.max(0,Math.min(5,n(state.ui.movieMinRating))),search=state.ui.movieSearch||'',typeFilter=state.ui.movieTypeFilter||'all',matchesType=m=>typeFilter==='all'||movieContentType(m.contentType,'기타')===typeFilter,allWish=state.movies.filter(m=>m.status!=='watched'),allWatched=state.movies.filter(m=>m.status==='watched'),wishRecords=sortRatedItems(allWish.filter(matchesType),'recent','watchedDate'),watchedRecords=sortRatedItems(filterMinimumRating(filterText(allWatched.filter(matchesType),search,['title','director','actors','review','contentType']),min),sort,'watchedDate'),wish=movieWorkGroups(wishRecords),watched=movieWorkGroups(watchedRecords),completedWatched=allWatched.filter(m=>completedByToday(m.watchedDate)),ratings=completedWatched.map(m=>ratingValue(m.rating)).filter(v=>v!==null),avg=ratings.length?ratings.reduce((a,b)=>a+b,0)/ratings.length:0,five=ratings.filter(v=>v===5).length;
  const watchedUnits=movieViewingUnits(watchedRecords),allUnits=movieViewingUnits(state.movies),wishUnits=movieViewingUnits(allWish);$('movieCount').textContent=allUnits+'편';$('wishMovieCount').textContent=wishUnits+'편';$('watchedMovieCount').textContent=`${watched.length}작품 · ${watchedUnits}편`;$('movieStats').innerHTML=[['전체 기록',allUnits+'편'],['관람 예정',wishUnits+'편'],['관람 완료',movieViewingUnits(completedWatched)+'편'],['평균 평점',avg?`${avg.toFixed(1)}점 · 만점 ${five}편`:'-']].map(([l,v])=>`<div class="stat"><div class="label">${l}</div><div class="value">${v}</div></div>`).join('');
  if($('movieSeriesSection'))$('movieSeriesSection').hidden=true;const size=uiPageSize(4,3),wishPage=uiPageSlice('movieWish',wish,size),watchedPage=uiPageSlice('movieWatched',watched,size);$('wishMovies').innerHTML=wishPage.rows.map(movieWorkCard).join('')||'<div class="empty">조건에 맞는 관람 예정작이 없습니다.</div>';$('watchedMovies').innerHTML=watchedPage.rows.map(movieWorkCard).join('')||'<div class="empty">조건에 맞는 관람 작품이 없습니다.</div>';uiMountPagination($('wishMovies'),'movieWish',wishPage,renderMovies,'작품');uiMountPagination($('watchedMovies'),'movieWatched',watchedPage,renderMovies,'작품');
  $('movieSort').value=sort;$('movieMinRating').value=min||'';$('movieSearch').value=search;$('movieTypeFilter').value=typeFilter;$('movieSort').onchange=()=>{state.ui.movieSort=$('movieSort').value;uiResetPage('movieWatched');save();renderMovies()};$('movieMinRating').oninput=()=>{state.ui.movieMinRating=Math.max(0,Math.min(5,n($('movieMinRating').value)));uiResetPage('movieWatched');save();renderMovies()};$('movieSearch').oninput=()=>{state.ui.movieSearch=$('movieSearch').value;uiResetPage('movieWatched');renderMovies()};$('movieTypeFilter').onchange=()=>{state.ui.movieTypeFilter=$('movieTypeFilter').value;uiResetPage('movieWish');uiResetPage('movieWatched');save();renderMovies()};updateMovieFormState();
  document.querySelectorAll('[data-movie-delete]').forEach(b=>b.onclick=()=>{if(!confirm('이 작품 기록을 삭제할까요?'))return;state.movies=state.movies.filter(x=>x.id!==b.dataset.movieDelete);commit('작품 기록을 삭제했습니다.')});document.querySelectorAll('[data-movie-toggle]').forEach(b=>b.onclick=()=>{const x=state.movies.find(v=>v.id===b.dataset.movieToggle);if(!x)return;x.status=x.status==='watched'?'wish':'watched';if(x.status==='watched'&&!x.watchedDate)x.watchedDate=today();x.updatedAt=new Date().toISOString();commit('작품 분류를 변경했습니다.')});document.querySelectorAll('[data-movie-edit]').forEach(b=>b.onclick=()=>{const x=state.movies.find(v=>v.id===b.dataset.movieEdit);if(!x)return;const meta=movieSeriesMeta(x);$('movieEditId').value=x.id;$('movieStatus').value=x.status||'wish';$('movieContentType').value=movieContentType(x.contentType,'기타');$('movieOrigin').value=x.origin||'국내';$('movieTitle').value=x.title||'';$('movieDirector').value=x.director||'';$('movieActors').value=x.actors||'';$('movieRating').value=x.rating??'';$('movieDate').value=x.watchedDate||'';$('movieSeason').value=meta?.season||'';$('movieEpisode').value=meta?.episode||'';$('movieReview').value=meta?movieReviewBody(x.review):x.review||'';$('saveMovie').textContent='수정 저장';updateMovieFormState();document.querySelector('#movie .phase1-collapsible-form')?.setAttribute('open','');$('movie').scrollIntoView({behavior:'smooth'})});
}
$("cancelMovieEdit").onclick=resetMovieForm;$("movieStatus").onchange=updateMovieFormState;$("movieContentType").onchange=updateMovieFormState;
$("saveMovie").onclick=async()=>{
  ...
  const contentType=movieContentType($("movieContentType").value,"영화"),season=Math.max(0,Math.floor(n($("movieSeason").value))),episode=Math.max(0,Math.floor(n($("movieEpisode").value))),series=["시리즈","드라마","애니메이션"].includes(contentType),progress=series?[season&&`시즌 ${season}`,episode&&`${episode}화까지`].filter(Boolean).join(" · "):"",reviewBody=$("movieReview").value.trim(),review=[progress,reviewBody].filter(Boolean).join("\n");
  const row={id:old?.id||uid(),status,contentType,origin:$("movieOrigin").value,title,director:$("movieDirector").value.trim(),actors:$("movieActors").value.trim(),rating:status==="watched"?enteredRating:(old?.rating??null),watchedDate:status==="watched"?$("movieDate").value:(old?.watchedDate||""),review,poster,createdAt:old?.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString()};
...
function diaryCard(d){return `<article class="diary-card phase1-post-row"><div class="diary-mood">${diaryMoodIcon(d.mood)}</div><div class="phase1-post-main"><div class="diary-date">${esc(d.date||'')} · ${d.updatedAt&&d.updatedAt!==d.createdAt?'수정됨':'작성'}</div><h4>${esc(d.title||'제목 없는 일기')}</h4><div class="diary-preview">${esc(d.content||'')}</div></div><div class="diary-actions"><button class="btn sm" data-diary-view="${d.id}">열어보기</button><button class="btn sm" data-diary-edit="${d.id}">수정</button><button class="btn sm danger" data-diary-delete="${d.id}">삭제</button></div></article>`}
...
function renderDiary(){
  const search=state.ui.diarySearch||'',sort=state.ui.diarySort||'newest',all=Array.isArray(state.diaries)?state.diaries:[];let rows=filterText(all,search,['title','content','date']);rows.sort((a,b)=>sort==='oldest'?(a.date||'').localeCompare(b.date||'')||(a.createdAt||'').localeCompare(b.createdAt||''):sort==='updated'?(b.updatedAt||'').localeCompare(a.updatedAt||''):(b.date||'').localeCompare(a.date||'')||(b.createdAt||'').localeCompare(a.createdAt||''));
  $('diaryCount').textContent=all.length+'개';
  const pageData=uiPageSlice('diary',rows,uiPageSize(8,5));
  $('diaryList').innerHTML=pageData.rows.map(diaryCard).join('')||'<div class="empty">조건에 맞는 일기가 없습니다.</div>';
  uiMountPagination($('diaryList'),'diary',pageData,renderDiary,'개');
  $('diarySearch').value=search;$('diarySort').value=sort;
  $('diarySearch').oninput=()=>{state.ui.diarySearch=$('diarySearch').value;uiResetPage('diary');renderDiary()};
  $('diarySort').onchange=()=>{state.ui.diarySort=$('diarySort').value;uiResetPage('diary');save();renderDiary()};
  document.querySelectorAll('[data-diary-view]').forEach(b=>b.onclick=()=>openDiaryDetail(b.dataset.diaryView));
  document.querySelectorAll('[data-diary-edit]').forEach(b=>b.onclick=()=>{document.querySelector('#diary .phase1-collapsible-form')?.setAttribute('open','');editDiary(b.dataset.diaryEdit)});
  document.querySelectorAll('[data-diary-delete]').forEach(b=>b.onclick=()=>deleteDiary(b.dataset.diaryDelete));
}
...
function renderTravelImportPreview(){const box=$("travelItineraryPreview"),btn=$("travelItineraryImport");if(!box||!btn)return;if(!travelImportPreviewRows.length){box.innerHTML='<div class="travel-import-empty">인식된 일정이 없습니다. 붙여넣은 범위와 헤더를 확인해 주세요.</div>';btn.disabled=true;return}box.innerHTML=`<div class="bulk-hint">실제로 방문한 가게·명소만 마지막 열에 체크하면 장소 목록에도 연결합니다. 계획만 적힌 행은 체크하지 마세요.</div><table><thead><tr><th>일차</th><th>시간</th><th>구분</th><th>장소</th><th>비고</th><th>예상비용</th><th>방문 장소</th></tr></thead><tbody>${travelImportPreviewRows.map((r,i)=>`<tr data-travel-preview="${i}"><td><input value="${esc(r.day||"")}" placeholder="1일차"></td><td><input value="${esc(r.time||"")}" placeholder="10:00"></td><td><input value="${esc(r.category||r.schedule||"")}" placeholder="관광"></td><td><input class="wide-input" value="${esc(r.place||"")}"></td><td><input class="note-input" value="${esc(r.note||"")}"></td><td><input value="${esc(r.costText||"")}" placeholder="₩10,000"></td><td><input type="checkbox" class="travel-preview-place" aria-label="${esc(r.place||"이 행")}을 방문 장소로 연결"></td></tr>`).join("")}</tbody></table>`;btn.disabled=false}
function readTravelImportPreview(){return [...$("travelItineraryPreview").querySelectorAll("[data-travel-preview]")].map(row=>{const v=[...row.querySelectorAll("input:not([type=checkbox])")].map(x=>x.value.trim()),original=travelImportPreviewRows[Number(row.dataset.travelPreview)],stop=normalizeTravelStop({day:v[0],date:original?.date||"",time:v[1],category:v[2],schedule:v[2],place:v[3],note:v[4],costText:v[5]});return {stop,link:!!row.querySelector(".travel-preview-place")?.checked}}).filter(x=>x.stop.time||x.stop.category||x.stop.place||x.stop.note||x.stop.costText)}
...
function openTravelStopModal(stop=null){
  $("travelStopEditId").value=stop?.id||"";
  $("travelStopModalTitle").textContent=stop?"일정 행 수정":"일정 행 추가";
  $("travelStopDay").value=stop?.day||"";
  $("travelStopDate").value=stop?.date||"";
  $("travelStopTime").value=stop?.time||"";
  $("travelStopCategory").value=stop?.category||stop?.schedule||"";
  $("travelStopPlace").value=stop?.place||"";
  $("travelStopNote").value=stop?.note||"";
  $("travelStopCost").value=stop?.costText||"";
  openModal("travelStopModal");
}
function moveTravelStop(t,id,delta){
  const from=t.itinerary.findIndex(x=>x.id===id),to=from+delta;
  if(from<0||to<0||to>=t.itinerary.length)return;
  const [row]=t.itinerary.splice(from,1);t.itinerary.splice(to,0,row);t.updatedAt=new Date().toISOString();
  commit("여행 일정 순서를 변경했습니다.",false);openTravelTripDetail(t.id);
}
function renderTravelDetailLists(t){
  $("travelStopCount").textContent=`${t.itinerary.length}개`;
  $("travelStopList").innerHTML=t.itinerary.length?`<div class="travel-itinerary-table-wrap"><table class="travel-itinerary-table"><thead><tr><th>일차</th><th>날짜·시간</th><th>구분</th><th>장소·일정</th><th>비고</th><th>예상 비용</th><th>관리</th></tr></thead><tbody>${t.itinerary.map((s,i)=>`<tr data-travel-stop="${esc(s.id)}"><td data-label="일차">${esc(s.day||"-")}</td><td data-label="날짜·시간"><b>${esc(s.time||"-")}</b>${s.date?`<small>${esc(s.date)}</small>`:""}</td><td data-label="구분">${esc(s.category||s.schedule||"일정")}</td><td data-label="장소·일정">${esc(s.place||"-")}</td><td data-label="비고">${esc(s.note||"-")}</td><td data-label="예상 비용">${esc(s.costText||"-")}</td><td data-label="관리"><div class="travel-row-actions"><button class="btn sm travel-stop-up" type="button" aria-label="위로 이동" ${i===0?"disabled":""}>↑</button><button class="btn sm travel-stop-down" type="button" aria-label="아래로 이동" ${i===t.itinerary.length-1?"disabled":""}>↓</button><button class="btn sm travel-stop-edit" type="button">수정</button><button class="btn sm danger travel-stop-delete" type="button">삭제</button></div></td></tr>`).join("")}</tbody></table></div>`:`<div class="empty">일정표가 비어 있어요. 일정 행을 직접 추가하거나 기존 시트 복붙 미리보기를 이용할 수 있습니다.</div>`;
  $("travelStopList").querySelectorAll("[data-travel-stop]").forEach(row=>{
    const id=row.dataset.travelStop;
    row.querySelector(".travel-stop-up").onclick=()=>moveTravelStop(t,id,-1);
    row.querySelector(".travel-stop-down").onclick=()=>moveTravelStop(t,id,1);
    row.querySelector(".travel-stop-edit").onclick=()=>openTravelStopModal(t.itinerary.find(x=>x.id===id));
    row.querySelector(".travel-stop-delete").onclick=()=>{if(!confirm("이 일정을 삭제할까요?"))return;t.itinerary=t.itinerary.filter(x=>x.id!==id);t.updatedAt=new Date().toISOString();commit("여행 일정을 삭제했습니다.",false);openTravelTripDetail(t.id)};
  });
  const linked=[...t.reviews.map(place=>({place,legacy:true})),...(state.travelPlaces||[]).filter(place=>place.tripId===t.id).map(place=>({place,legacy:false}))];
  $("travelReviewCount").textContent=`${linked.length}개`;
  $("travelReviewList").innerHTML=linked.length?linked.map(({place:r,legacy})=>`<div class="travel-review" data-travel-review="${esc(r.id)}"><div class="travel-review-head"><div><b>${travelArchiveIcon(r.type)} ${esc(r.name)}</b><div class="travel-review-meta">${travelReviewMetaHtml(r)}${r.highlight?` · ${esc(r.highlight)}`:""}</div></div><button class="btn sm danger ${legacy?"travel-review-delete":"travel-linked-place-delete"}" type="button">삭제</button></div>${r.review?`<div class="travel-review-body">${esc(r.review)}</div>`:""}</div>`).join(""):`<div class="empty">아직 연결된 맛집·명소 기록이 없어요.</div>`;
  $("travelReviewList").querySelectorAll(".travel-review-delete").forEach(btn=>btn.onclick=()=>{if(!confirm("이 여행 아카이브 기록을 삭제할까요?"))return;t.reviews=t.reviews.filter(x=>x.id!==btn.closest("[data-travel-review]").dataset.travelReview);t.updatedAt=new Date().toISOString();commit("여행 아카이브 기록을 삭제했습니다.",false);openTravelTripDetail(t.id)});
  $("travelReviewList").querySelectorAll(".travel-linked-place-delete").forEach(btn=>btn.onclick=()=>{const id=btn.closest("[data-travel-review]").dataset.travelReview,p=state.travelPlaces.find(x=>x.id===id);if(!p||!confirm(`'${p.name}' 장소 기록을 삭제할까요? 삭제 전 전체 백업을 생성합니다.`))return;exportData({suffix:"before_delete_travel_place",silent:true});const before=state.travelPlaces;state.travelPlaces=before.filter(x=>x.id!==id);if(!commit("장소 기록을 삭제했습니다.",false)){state.travelPlaces=before;renderAll()}openTravelTripDetail(t.id)});}
function travelArchiveGroup(type){const t=String(type||"").trim();if(t==="식당"||t==="음식")return "식당";if(t==="카페")return "카페";if(t==="숙소")return "숙소";if(t==="관광/장소"||t==="명소")return "명소";if(t==="쇼핑")return "쇼핑";return "기타"}
function travelPlaceRows(trips=state.travelTrips||[]){return [...(state.travelPlaces||[]).map(place=>({place,trip:trips.find(t=>t.id===place.tripId)||null,legacy:false})),...trips.flatMap(t=>(t.reviews||[]).map(place=>({place,trip:t,legacy:true})))]}
function travelPlaceHighlightLabel(){const type=$("travelPlaceType")?.value;$("travelPlaceHighlightLabel").textContent=type==="식당"||type==="카페"?"대표 메뉴":type==="명소"?"볼거리 · 체험":type==="숙소"?"객실 · 시설":type==="쇼핑"?"구매 품목":"기억할 점"}
function openTravelPlaceModal(place=null,tripId=""){
  $("travelPlaceEditId").value=place?.id||"";$("travelPlaceModalTitle").textContent=place?"장소 수정":"장소 등록";
  $("travelPlaceType").value=["식당","카페","명소","숙소","쇼핑","기타"].includes(place?.type)?place.type:"식당";
  $("travelPlaceName").value=place?.name||"";$("travelPlaceDate").value=place?.visitDate||"";$("travelPlaceLocation").value=place?.location||"";
  $("travelPlaceRating").value=place?.rating||"";$("travelPlaceCost").value=place?.cost||"";$("travelPlaceHighlight").value=place?.highlight||"";
  $("travelPlacePriceRange").value=place?.priceRange||"";$("travelPlaceReview").value=place?.review||"";$("travelPlaceReturn").value=place?.returnVisit||"maybe";
  const trips=state.travelTrips||[];$("travelPlaceTrip").innerHTML='<option value="">여행과 연결하지 않음</option>'+trips.map(t=>`<option value="${esc(t.id)}">${esc(t.name||t.destination||"이름 없는 여행")}</option>`).join("");
  $("travelPlaceTrip").value=place?.tripId||tripId||"";travelPlaceHighlightLabel();openModal("travelPlaceModal");
}
function travelPlaceExistsForStop(tripId,stop){const name=String(stop.place||"").trim().toLocaleLowerCase("ko-KR");return (state.travelPlaces||[]).some(p=>p.tripId===tripId&&(p.sourceStopId===stop.id||(name&&p.name.toLocaleLowerCase("ko-KR")===name)))}
function travelPlaceTypeFromStop(category){const text=String(category||"");return /카페|커피/.test(text)?"카페":/식사|식당|맛집|음식|점심|저녁|아침/.test(text)?"식당":/숙소|호텔/.test(text)?"숙소":/쇼핑|시장/.test(text)?"쇼핑":/관광|명소|전시|박물관|공원|체험/.test(text)?"명소":"기타"}
function renderTravelArchive(trips){
  const all=travelPlaceRows(trips),typeSel=$("travelArchiveType"),tripSel=$("travelArchiveTrip"),sortSel=$("travelArchiveSort"),search=$("travelArchiveSearch");if(!typeSel||!tripSel)return;
  const types=["식당","카페","숙소","명소","쇼핑","기타"],wantedType=state.ui.travelArchiveType||"all",wantedTrip=state.ui.travelArchiveTrip||"all";
  typeSel.innerHTML='<option value="all">전체 카테고리</option>'+types.map(v=>`<option value="${v}">${v}</option>`).join("");
  tripSel.innerHTML='<option value="all">전체 장소</option><option value="independent">여행 미연결</option>'+trips.map(t=>`<option value="${esc(t.id)}">${esc(t.name||t.destination||"이름 없는 여행")}</option>`).join("");
  typeSel.value=types.includes(wantedType)?wantedType:"all";tripSel.value=wantedTrip==="independent"||trips.some(t=>t.id===wantedTrip)?wantedTrip:"all";state.ui.travelArchiveType=typeSel.value;state.ui.travelArchiveTrip=tripSel.value;
  sortSel.value=["recent","rating","name"].includes(state.ui.travelArchiveSort)?state.ui.travelArchiveSort:"recent";search.value=state.ui.travelArchiveSearch||"";
  const q=search.value.trim().toLocaleLowerCase("ko-KR"),rows=all.filter(({place:p,trip:t})=>(typeSel.value==="all"||travelArchiveGroup(p.type)===typeSel.value)&&(tripSel.value==="all"||(tripSel.value==="independent"?!t:t?.id===tripSel.value))&&(!q||[p.name,p.location,p.highlight,p.review].some(v=>String(v||"").toLocaleLowerCase("ko-KR").includes(q)))).sort((a,b)=>sortSel.value==="rating"?(n(b.place.rating)-n(a.place.rating)||String(b.place.visitDate||"").localeCompare(String(a.place.visitDate||""))):sortSel.value==="name"?String(a.place.name||"").localeCompare(String(b.place.name||""),"ko-KR"):String(b.place.visitDate||b.place.createdAt||"").localeCompare(String(a.place.visitDate||a.place.createdAt||"")));
  $("travelArchiveCount").textContent=`${rows.length}개`;
  $("travelArchiveGrid").innerHTML=rows.length?rows.map(({trip:t,place:p,legacy})=>`<article class="travel-place-row"><div class="travel-place-primary"><span class="travel-archive-type">${travelArchiveIcon(p.type)} ${esc(travelArchiveGroup(p.type))}</span><div><h4>${esc(p.name||"이름 없는 기록")}</h4><small>${esc(p.location||"위치 미입력")}${p.visitDate?` · ${esc(p.visitDate)}`:""}</small></div></div><div class="travel-place-rating" aria-label="내 평점">${p.rating?`★ ${n(p.rating).toFixed(1)}`:"평점 미입력"}</div><div class="travel-place-detail"><b>${esc(p.highlight||p.review||"메뉴·볼거리 미입력")}</b><small>${esc(p.priceRange||travelCostLabel(p.cost)||"가격 미입력")}</small></div><div class="travel-place-source"><small>${t?`여행 · ${esc(t.name||t.destination||"이름 없는 여행")}`:legacy?"기존 여행 기록":"독립 등록"}</small><small>${esc(travelReturnLabel(p.returnVisit))}</small></div><div class="travel-place-actions">${legacy?`<button class="btn sm travel-archive-open" data-trip-id="${esc(t.id)}" type="button">원본 여행</button>`:`<button class="btn sm travel-place-edit" data-place-id="${esc(p.id)}" type="button">수정</button><button class="btn sm danger travel-place-delete" data-place-id="${esc(p.id)}" type="button">삭제</button>`}</div></article>`).join(""):'<div class="empty">조건에 맞는 장소가 없어요. 여행과 관계없이 첫 장소를 등록해 보세요.</div>';
  $("travelArchiveGrid").querySelectorAll(".travel-archive-open").forEach(btn=>btn.onclick=()=>openTravelTripDetail(btn.dataset.tripId));
  $("travelArchiveGrid").querySelectorAll(".travel-place-edit").forEach(btn=>btn.onclick=()=>openTravelPlaceModal(state.travelPlaces.find(p=>p.id===btn.dataset.placeId)));
  $("travelArchiveGrid").querySelectorAll(".travel-place-delete").forEach(btn=>btn.onclick=()=>{const p=state.travelPlaces.find(x=>x.id===btn.dataset.placeId);if(!p||!confirm(`'${p.name}' 장소 기록을 삭제할까요? 삭제 전 전체 백업을 생성합니다.`))return;exportData({suffix:"before_delete_travel_place",silent:true});const before=state.travelPlaces;state.travelPlaces=before.filter(x=>x.id!==p.id);if(!commit("장소 기록을 삭제했습니다.")){state.travelPlaces=before;renderAll()}});
  typeSel.onchange=()=>{state.ui.travelArchiveType=typeSel.value;renderTravel()};tripSel.onchange=()=>{state.ui.travelArchiveTrip=tripSel.value;renderTravel()};sortSel.onchange=()=>{state.ui.travelArchiveSort=sortSel.value;renderTravel()};search.oninput=()=>{state.ui.travelArchiveSearch=search.value;renderTravel()};
}
function renderTravel(){
  if(!$("travelKpis"))return;
  const trips=[...(state.travelTrips||[])].sort((a,b)=>String(b.startDate||"").localeCompare(String(a.startDate||""))),wish=[...(state.travelWishlist||[])].sort((a,b)=>String(a.expectedDate||"9999").localeCompare(String(b.expectedDate||"9999"))),tab=state.ui.travelTab||"trips";
  document.querySelectorAll("[data-travel-tab]").forEach(b=>b.classList.toggle("active",b.dataset.travelTab===tab));$("travelTripsPane").classList.toggle("hidden",tab!=="trips");$("travelArchivePane").classList.toggle("hidden",tab!=="archive");$("travelWishPane").classList.toggle("hidden",tab!=="wish");
  const places=travelPlaceRows(trips),rated=places.map(x=>x.place).filter(r=>n(r.rating)>0),avg=rated.length?rated.reduce((a,r)=>a+n(r.rating),0)/rated.length:0,destinations=new Set(trips.map(t=>String(t.destination||"").trim()).filter(Boolean)),nextWish=wish.find(w=>w.expectedDate&&w.expectedDate>=today())||wish[0],foodCount=places.filter(x=>["식당","카페"].includes(travelArchiveGroup(x.place.type))).length;
  $("travelKpis").innerHTML=tab==="archive"?`<div class="module-kpi"><small>기록한 장소</small><b>${places.length}곳</b><div class="sub">독립 등록 + 여행 후기</div></div><div class="module-kpi"><small>식당·카페</small><b>${foodCount}곳</b><div class="sub">내 맛집 목록</div></div><div class="module-kpi"><small>명소·기타</small><b>${places.length-foodCount}곳</b><div class="sub">다녀온 장소</div></div><div class="module-kpi"><small>내 평균 평점</small><b>${rated.length?avg.toFixed(1):"-"}</b><div class="sub">평점 ${rated.length}개 기준</div></div>`:`<div class="module-kpi"><small>완료 여행</small><b>${trips.length}회</b><div class="sub">기존 여행 원본</div></div><div class="module-kpi"><small>기록한 지역</small><b>${destinations.size}곳</b><div class="sub">목적지 기준</div></div><div class="module-kpi"><small>후기 평균</small><b>${rated.length?avg.toFixed(1):"-"}</b><div class="sub">${places.length}개 장소 기록</div></div><div class="module-kpi"><small>다음 여행</small><b>${nextWish?esc(nextWish.destination):"아직 없음"}</b><div class="sub">${nextWish?.expectedDate?esc(nextWish.expectedDate):"자유 메모부터 시작해요"}</div></div>`;
  $("travelTripCount").textContent=`${trips.length}회`;
  $("travelTripGrid").innerHTML=trips.length?trips.map(t=>{const tPlaces=places.filter(x=>x.trip?.id===t.id).map(x=>x.place),tRated=tPlaces.filter(r=>n(r.rating)>0),tAvg=tRated.length?tRated.reduce((a,r)=>a+n(r.rating),0)/tRated.length:0;return `<article class="travel-card" data-travel-trip="${esc(t.id)}"><span class="travel-destination">${esc(t.destination||"목적지 미등록")}</span><h4>${esc(t.name||t.destination||"이름 없는 여행")}</h4><div class="travel-card-meta"><span>${esc(travelDateRange(t))}</span>${t.companions?`<span>· 동행 ${esc(t.companions)}</span>`:""}</div><div class="travel-card-note">${esc(t.summary||"이번 여행의 한줄평을 남겨보세요.")}</div><div class="travel-plan-chips"><span class="travel-plan-chip">일정 ${t.itinerary.length}</span><span class="travel-plan-chip">후기 ${tPlaces.length}</span>${tRated.length?`<span class="travel-plan-chip warm">★ ${tAvg.toFixed(1)}</span>`:""}${t.sourceWishId?'<span class="travel-plan-chip">Wish 전환</span>':""}</div><div class="travel-card-actions"><button class="btn sm primary travel-trip-open" type="button">상세·일정 보기</button><button class="btn sm travel-trip-edit" type="button">수정</button></div></article>`}).join(""):`<div class="empty" style="grid-column:1/-1">완료 여행 기록이 없어요. 여행 기본정보부터 가볍게 등록해 보세요.</div>`;
  $("travelTripGrid").querySelectorAll(".travel-trip-open").forEach(btn=>btn.onclick=()=>openTravelTripDetail(btn.closest("[data-travel-trip]").dataset.travelTrip));$("travelTripGrid").querySelectorAll(".travel-trip-edit").forEach(btn=>btn.onclick=()=>openTravelTripModal(state.travelTrips.find(x=>x.id===btn.closest("[data-travel-trip]").dataset.travelTrip)));renderTravelArchive(trips);
  $("travelWishCount").textContent=`${wish.length}곳`;
  $("travelWishGrid").innerHTML=wish.length?wish.map(w=>`<article class="travel-card travel-wish-card" data-travel-wish="${esc(w.id)}"><span class="wish-date">${w.expectedDate?`예상 ${esc(w.expectedDate)}`:"날짜 미정"}</span><h4>${esc(w.destination)}</h4><div class="travel-wish-copy">${w.reason?`<b>가고 싶은 이유</b><p>${esc(w.reason)}</p>`:w.note?`<b>자유 메모</b><p>${esc(w.note)}</p>`:`<p>가고 싶은 이유나 메모부터 자유롭게 적어보세요.</p>`}</div>${w.reason&&w.note?`<div class="travel-card-note">${esc(w.note)}</div>`:""}<div class="travel-plan-chips">${w.transport?'<span class="travel-plan-chip">교통</span>':""}${w.places?'<span class="travel-plan-chip">장소</span>':""}${w.foods||w.restaurants?'<span class="travel-plan-chip">음식</span>':""}${w.lodging?'<span class="travel-plan-chip">숙소</span>':""}${w.itinerary?.length?`<span class="travel-plan-chip">일정 ${w.itinerary.length}</span>`:""}</div><div class="travel-card-actions"><button class="btn sm primary travel-wish-convert" type="button">완료 여행으로 전환</button><button class="btn sm travel-wish-edit" type="button">계획 수정</button><button class="btn sm danger travel-wish-delete" type="button">삭제</button></div></article>`).join(""):`<div class="empty" style="grid-column:1/-1">갈 여행 후보가 비어 있어요. 자유 메모를 먼저 남기거나 후보를 추가해 보세요.</div>`;
  $("travelWishGrid").querySelectorAll(".travel-wish-edit").forEach(btn=>btn.onclick=()=>openTravelWishModal(state.travelWishlist.find(x=>x.id===btn.closest("[data-travel-wish]").dataset.travelWish)));$("travelWishGrid").querySelectorAll(".travel-wish-delete").forEach(btn=>btn.onclick=()=>{const id=btn.closest("[data-travel-wish]").dataset.travelWish,w=state.travelWishlist.find(x=>x.id===id);if(!w||!confirm(`Wish '${w.destination}'을 삭제할까요?\n삭제 전 자동 백업을 생성합니다.`))return;exportData({suffix:"before_delete_travel_wish",silent:true});state.travelWishlist=state.travelWishlist.filter(x=>x.id!==id);commit("Wish를 삭제했습니다.")});$("travelWishGrid").querySelectorAll(".travel-wish-convert").forEach(btn=>btn.onclick=()=>convertTravelWish(btn.closest("[data-travel-wish]").dataset.travelWish));$("travelQuickNote").value=state.pageNotes?.travel||"";
}
...
function wishlistAiReviewLabel(row={}){return row.sourceType==='agent'?'AI 검토 완료':['conversation','intake'].includes(row.sourceType)?'AI 접수 후보':'AI 검토 없음'}
...
function renderWishlist(){
  if(!$('wishlistGrid'))return;const all=[...(state.wishlistItems||[])],status=state.ui.wishlistFilter||'all',kind=state.ui.wishlistKindFilter||'all',q=(state.ui.wishlistSearch||'').trim().toLocaleLowerCase('ko-KR'),rows=all.filter(x=>(status==='all'||x.status===status)&&(kind==='all'||x.kind===kind)&&(!q||[x.name,x.category,x.reason,x.note].join(' ').toLocaleLowerCase('ko-KR').includes(q))).sort((a,b)=>({high:0,medium:1,low:2}[a.priority]??1)-({high:0,medium:1,low:2}[b.priority]??1)||String(b.updatedAt||'').localeCompare(String(a.updatedAt||'')));
  $('wishlistCount').textContent=all.length+'개';$('wishlistFilter').value=status;$('wishlistKindFilter').value=kind;$('wishlistSearch').value=state.ui.wishlistSearch||'';const counts={consider:0,planned:0,purchased:0,hold:0};all.forEach(x=>{if(counts[x.status]!==undefined)counts[x.status]++});[['wishlistKpiAll',all.length],['wishlistKpiConsider',counts.consider],['wishlistKpiPlanned',counts.planned],['wishlistKpiPurchased',counts.purchased]].forEach(([id,v])=>{if($(id))$(id).textContent=String(v)});
  const pageData=uiPageSlice('wishlist',rows,uiPageSize(8,5));$('wishlistGrid').innerHTML=pageData.rows.length?pageData.rows.map(x=>`<article class="wishlist-item-card phase1-post-row" data-wish-id="${esc(x.id)}"><div class="phase1-post-main"><div class="wishlist-item-badges"><span class="wishlist-kind ${esc(x.kind)}">${esc(wishlistKindLabel(x.kind))}</span><span class="wishlist-status ${esc(x.status)}">${esc(wishlistStatusLabel(x.status))}</span><span class="wishlist-priority ${esc(x.priority)}">우선 ${esc(wishlistPriorityLabel(x.priority))}</span><span class="wishlist-ai ${x.sourceType==='agent'?'reviewed':'pending'}">${esc(wishlistAiReviewLabel(x))}</span></div><h4>${esc(x.name||'이름 없는 Wish')}</h4><div class="wishlist-item-meta">${esc(x.category||'기타')}${x.price!==null?` · 예상 ${won(x.price)}`:' · 가격 미정'}</div><p class="wishlist-preview">${esc(x.reason||x.note||'상세 메모가 없습니다.')}</p><details><summary>상세 보기</summary><div class="wishlist-review-state"><b>AI 검토</b><p>${esc(wishlistAiReviewLabel(x))}${x.sourceLabel?` · ${esc(x.sourceLabel)}`:''}</p></div>${x.reason?`<div class="wishlist-reason"><b>왜 갖고 싶은가</b><p>${esc(x.reason)}</p></div>`:''}${x.note?`<div class="wishlist-note">${esc(x.note)}</div>`:''}${x.status==='purchased'&&x.purchasedDate?`<div class="wishlist-complete">완료 ${esc(x.purchasedDate)}</div>`:''}</details></div><div class="wishlist-item-actions"><button class="btn sm" data-wish-edit="${esc(x.id)}">수정</button><button class="btn sm danger" data-wish-delete="${esc(x.id)}">삭제</button></div></article>`).join(''):'<div class="empty">아직 Wish가 없습니다. 직접 추가하거나 유나에게 대화를 붙여넣고 후보를 받아보세요.</div>';uiMountPagination($('wishlistGrid'),'wishlist',pageData,renderWishlist,'개');
  $('wishlistGrid').querySelectorAll('[data-wish-edit]').forEach(b=>b.onclick=()=>{const x=state.wishlistItems.find(r=>r.id===b.dataset.wishEdit);if(x)openWishlistModal(x)});$('wishlistGrid').querySelectorAll('[data-wish-delete]').forEach(b=>b.onclick=()=>{const x=state.wishlistItems.find(r=>r.id===b.dataset.wishDelete);if(!x||!confirm(`'${x.name}' Wish를 삭제할까요?`))return;state.wishlistItems=state.wishlistItems.filter(r=>r.id!==x.id);commit('Wish-list 항목을 삭제했습니다.')});
}
...
const HANI_DISPLAY_VERSION="2.9.134";
...
function agentTravelSnapshot(){return {completed_trips:(state.travelTrips||[]).slice(-6).map(t=>({name:t.name,destination:t.destination,startDate:t.startDate,endDate:t.endDate,summary:agentSafeText(t.summary,140),review_count:(t.reviews||[]).length+(state.travelPlaces||[]).filter(p=>p.tripId===t.id).length,reviews:[...(t.reviews||[]),...(state.travelPlaces||[]).filter(p=>p.tripId===t.id)].slice(-5).map(r=>({type:r.type,name:r.name,rating:r.rating,returnVisit:r.returnVisit,review:agentSafeText(r.review,100)}))})),places:(state.travelPlaces||[]).slice(-10).map(p=>({type:p.type,name:p.name,location:p.location,rating:p.rating,tripId:p.tripId,highlight:agentSafeText(p.highlight,80)})),wishlist:(state.travelWishlist||[]).slice(-8).map(w=>({destination:w.destination,expectedDate:w.expectedDate,reason:agentSafeText(w.reason||w.note,100)}))};}
...
async function agentLoadCases({selectId=""}={}){
  ...
  const nextId=selectId||agentActiveCaseId||agentCasesCache[0]?.id||"";if(nextId)await agentLoadCaseDetail(nextId);else if($("agentCaseDetail")){$("agentCaseDetail").innerHTML='<div><div style="font-size:34px;margin-bottom:8px">📄</div><b>아직 등록된 Agent 안건이 없습니다.</b></div>';agentDetailCache=null;window.haniOfficeLiveUpdate?.(null)}
}
...
const AGENT_READINESS_LABELS={READY:"READY",NEED_USER_INFO:"대표 답변 필요",NEED_RESEARCH:"자료 확인 필요",UNRESOLVED:"중요 이견 미해결",HOLD:"HOLD"};
function agentQuestionItems(v,h){
  const source=Array.isArray(v?.human_required_questions)?v.human_required_questions:(Array.isArray(v?.consolidated_questions)?v.consolidated_questions:agentArray(h?.representative_questions));
  return source.map((raw,index)=>{const item=agentObj(raw),text=String(typeof raw==="string"?raw:(item.question||item.text||item.label||"")).trim(),kindRaw=String(item.kind||item.priority||item.question_type||item.classification||"BLOCKING").toUpperCase(),kind=["BLOCKING","USEFUL","OPTIONAL"].includes(kindRaw)?kindRaw:"BLOCKING";return {id:String(item.id||`q-${index+1}`),text,kind}}).filter(item=>item.text);
}
function agentDecisionReadiness(c,h,v){
  const questions=agentQuestionItems(v,h),blocking=questions.filter(item=>item.kind==="BLOCKING"),conflicts=agentArray(h?.key_conflicts).length?agentArray(h.key_conflicts):agentArray(v?.conflicts),researchItems=[...agentArray(v?.research_required_items),...agentArray(h?.research_required_items)],explicit=String(v?.decision_status||v?.readiness_status||h?.decision_status||"").toUpperCase(),ready=Boolean(h?.ready_for_decision)&&c?.verification_status==="PASS";
  let status="HOLD";
  if(blocking.length||explicit==="NEED_USER_INFO")status="NEED_USER_INFO";
  else if(explicit==="NEED_RESEARCH"||v?.research_blocking===true)status="NEED_RESEARCH";
  else if(explicit==="UNRESOLVED"||(conflicts.length&&!ready))status="UNRESOLVED";
  else if(explicit==="HOLD")status="HOLD";
  else if(ready)status="READY";
  else if(c?.status==="HELD")status="HOLD";
  return {status,ready:status==="READY",questions,blocking,conflicts,researchItems,label:AGENT_READINESS_LABELS[status]||status};
}
function agentReadinessDescription(readiness){
  if(readiness.status==="READY")return "결론을 바꿀 핵심 질문과 검증을 통과해 대표 결재가 가능합니다.";
  if(readiness.status==="NEED_USER_INFO")return "결론에 영향을 주는 정보가 남아 있습니다. 답변은 같은 Case에 누적됩니다.";
  if(readiness.status==="NEED_RESEARCH")return "최신 자료 확인 전에는 실행 가능한 결론으로 확정하지 않습니다.";
  if(readiness.status==="UNRESOLVED")return "중요한 Agent 이견이 남아 있어 승인 대신 재검토가 필요합니다.";
  return "현재 정보로는 실행 가능한 결론을 확정하지 않습니다. Case는 보존되며 이어서 검토할 수 있습니다.";
}
...
function agentExecutiveSummaryHtml(c,h,v,latestDecision){
  const key='hani',member=teamByKey(key)||{},meta=teamMeta(key)||{},img=(sidebarAgentImages[key]||agentImages[key]||""),tone=member.tone||'purple',hasDecision=Object.keys(agentObj(h)).length>0,recommendation=h.recommendation||"NEEDS_DATA",summary=esc(hasDecision?h.executive_summary:(v.summary||v.blocker_reason||"결론을 만들기 전에 필요한 정보를 확인하고 있습니다.")),verdict=esc(AGENT_VERDICT_LABELS[recommendation]||recommendation),tags=agentArray(meta.signature).slice(0,3),role=esc(member.role||meta.rank||"HANI OS Manager"),readiness=agentDecisionReadiness(c,h,v),draft=!hasDecision||!readiness.ready;
  return `<div class="agent-summary verdict-${String(recommendation).toLowerCase()} tone-${tone} agent-hani ${draft?'is-readiness-draft':''}"><div class="agent-summary-topline"><div class="agent-summary-heading"><span class="agent-summary-kicker">${draft?'DECISION READINESS GATE':'HANI EXECUTIVE REVIEW'}</span><strong class="agent-summary-title">${draft?'결정 준비 점검':'하니 최종 결론'}</strong></div><div class="agent-summary-badges"><span class="agent-status ${readiness.ready?'ok':'warn'}">${esc(readiness.label)}</span>${hasDecision?`<span class="agent-summary-verdict verdict-${String(recommendation).toLowerCase()}">${verdict}</span>`:""}${latestDecision?`<span class="agent-summary-rep-decision">대표 ${esc(AGENT_DECISION_LABELS[latestDecision.representative_decision]||latestDecision.representative_decision)}</span>`:""}</div></div><div class="agent-summary-dialog"><div class="agent-summary-avatar" style="${img?`--agent-photo:url('${img}')`:''}"></div><div class="agent-summary-dialog-body"><div class="agent-summary-speaker"><div class="agent-summary-name"><b>${esc(member.name||'하니')}</b><small>${role}</small></div></div><div class="agent-summary-bubble"><div class="agent-summary-quote"><span class="agent-quote-name">${esc(member.name||'하니')}</span>“${summary}”</div></div>${draft?`<div class="agent-readiness-draft-note">${esc(agentReadinessDescription(readiness))}</div>`:""}${tags.length?`<div class="agent-summary-tags">${tags.map(t=>`<span>${esc(t)}</span>`).join("")}</div>`:""}</div></div></div>`;
}
...
function agentActionGuideHtml(c,h,v,reviews){
  const recommendation=h.recommendation||"NEEDS_DATA",readiness=agentDecisionReadiness(c,h,v),ready=readiness.ready,conditional=agentArray(v.conditional_checks),items=[];let title="지금 해야 할 일",desc=agentReadinessDescription(readiness),badge=readiness.label;
  ...
  else if(readiness.status==="NEED_USER_INFO"){title="대표 추가정보 필요";readiness.blocking.slice(0,2).forEach(q=>items.push(q.text));if(readiness.blocking.length>2)items.push(`나머지 ${readiness.blocking.length-2}개 질문은 이번 답변 반영 뒤 우선순위를 다시 확인합니다.`);}
  else if(["ANALYZING","DRAFT"].includes(c.status)){title="현재 상태";badge="분석 진행";desc="AI TEAM이 안건을 검토 중입니다. 아래 내용이 채워지면 결재 판단 단계로 넘어갑니다.";items.push("전문 Agent 의견이 쌓이는 중입니다.");items.push("하니 종합안과 Verify 결과가 생성되면 대표 결재 Gate가 열립니다.");}
  else if(readiness.status==="NEED_RESEARCH"){title="최신 자료 확인 필요";readiness.researchItems.slice(0,3).forEach(x=>items.push(typeof x==="string"?x:JSON.stringify(x)));}
  else if(readiness.status==="UNRESOLVED"){title="중요 이견 재검토 필요";readiness.conflicts.slice(0,3).forEach(x=>items.push(typeof x==="string"?x:JSON.stringify(x)));}
  else if(!ready){title="결론 보류";items.push(`Verify 상태: ${c.verification_status||'-'}`);if(conditional.length)conditional.slice(0,3).forEach(x=>items.push(typeof x==="string"?x:JSON.stringify(x)));else items.push("결론을 확정할 근거가 부족해 Case를 보존한 채 대기합니다.");}
  ...
  return `<div class="agent-next-action" data-readiness="${esc(readiness.status)}"><div class="agent-next-action-head"><div><b>${esc(title)}</b><span>${esc(desc)}</span></div><span class="agent-status ${ready?"ok":"warn"}">${esc(badge)}</span></div><div class="agent-next-action-list">${items.map((item,i)=>`<div class="agent-next-action-item"><i>${i+1}</i><div>${esc(item)}</div></div>`).join("")}</div></div>`;
}
...
function agentQuestionsHtml(v,h,c){
  const questions=agentQuestionItems(v,h),blocking=questions.filter(item=>item.kind==="BLOCKING"),asking=blocking.slice(0,2);if(!questions.length&&c.status==="HELD")return `<div class="agent-subsection agent-readiness-questions"><div class="agent-subsection-title"><b>같은 Case에 조건 추가</b><span class="agent-status warn">HOLD</span></div><div class="agent-question agent-additional-condition"><div class="agent-question-label"><span>NEW CONTEXT</span><b>결정에 필요한 정보나 새 조건</b></div><textarea id="agentAdditionalCondition" placeholder="예: 호텔은 난바역 근처로 할게."></textarea></div><div class="form-actions"><button class="btn primary" id="agentSubmitAnswers" type="button">조건 반영하고 재검토</button><button class="btn" id="agentOptionalGateRecheck" type="button">현재 정보로 다시 확인</button></div></div>`;if(!questions.length)return "";
...
function renderAgentCaseDetail(){
  const root=$("agentCaseDetail");if(!root||!agentDetailCache)return;const {case:c,reviews,decisions,events}=agentDetailCache,h=agentObj(c.hani_final),v=agentObj(c.verification),latestDecision=decisions[0]||null,recommendation=h.recommendation||"NEEDS_DATA",readiness=agentDecisionReadiness(c,h,v),ready=readiness.ready,hasExecutive=Object.keys(h).length>0,canDecide=hasExecutive&&ready&&c.status==="AWAITING_APPROVAL",canApprove=canDecide,overrideMode=false,conditional=agentArray(v.conditional_checks),conflicts=readiness.conflicts,required=agentArray(h.required_next_actions),wishEligible=c.workflow==="PURCHASE_REVIEW"&&latestDecision?.representative_decision==="APPROVE",wishSynced=wishEligible&&agentWishAlreadySynced(c),lifeStatus=wishSynced?"Wish-list 반영 완료":wishEligible?"Wish-list 반영 대기":"결정 이력 저장";
...
async function agentRecordDecision(decision){
  ...
  if(decision==="APPROVE"){const readiness=agentDecisionReadiness(c,agentObj(c.hani_final),agentObj(c.verification));if(!readiness.ready)return alert(`${readiness.label} 상태입니다. 결론을 바꿀 수 있는 질문과 검증을 먼저 완료해 주세요.`)}
...
async function agentFinalizeWithCurrentContext(){
  const c=agentDetailCache?.case;if(!c)return;
  const readiness=agentDecisionReadiness(c,agentObj(c.hani_final),agentObj(c.verification));if(readiness.blocking.length)return alert(`BLOCKING 질문 ${readiness.blocking.length}개가 남아 있어 현재 정보로 결론을 확정할 수 없습니다.`);
  try{
    agentSetBusy(true,"Decision Readiness 재확인 중");
    const result=await agentApi("verify_and_synthesize",{case_id:c.id,force_reverify:true});
    toast(result.message||"현재 정보로 Decision Readiness를 다시 확인했습니다.");await agentLoadCases({selectId:c.id});
  }catch(e){console.error("Decision readiness recheck",e);alert("Decision Readiness 확인 중 오류가 발생했습니다.\n"+(e?.message||e))}
  finally{agentSetBusy(false)}
}
async function agentRecheckReadiness(){
  const c=agentDetailCache?.case;if(!c)return;
  try{agentSetBusy(true,"Decision Readiness 재확인 중");const result=await agentApi("verify_and_synthesize",{case_id:c.id,force_reverify:true});toast(result.message||"Decision Readiness를 다시 확인했습니다.");await agentLoadCases({selectId:c.id})}catch(e){console.error("Decision readiness recheck",e);alert("Decision Readiness 확인 중 오류가 발생했습니다.\n"+(e?.message||e))}finally{agentSetBusy(false)}
}
...
async function agentSubmitNewRequest(){
  ...
  try{
    ...
    // Clarification belongs to a persisted Case. Do not ask disposable prompt()
    // questions before create_case; the readiness gate will rank true blockers.
    if(questions.length)router.initial_ambiguity_questions=questions.slice(0,2);
...
if($("investmentNewsEntityFilter"))$("investmentNewsEntityFilter").onchange=()=>{uiResetPage('investmentNews');state.ui.investmentNewsEntity=$("investmentNewsEntityFilter").value;save();renderInvestmentNews()};
if($("investmentNewsGradeFilter"))$("investmentNewsGradeFilter").onchange=()=>{uiResetPage('investmentNews');state.ui.investmentNewsGrade=$("investmentNewsGradeFilter").value;save();renderInvestmentNews()};
if($("investmentNewsSentimentFilter"))$("investmentNewsSentimentFilter").onchange=()=>{uiResetPage('investmentNews');state.ui.investmentNewsSentiment=$("investmentNewsSentimentFilter").value;save();renderInvestmentNews()};
...
$("travelPlaceAdd").onclick=()=>openTravelPlaceModal();
$("travelPlaceType").onchange=travelPlaceHighlightLabel;
$("travelPlaceSave").onclick=()=>{
  const name=$("travelPlaceName").value.trim(),ratingRaw=$("travelPlaceRating").value.trim(),costRaw=$("travelPlaceCost").value.trim();
  if(!name)return alert("장소 이름을 입력해 주세요.");
  if(ratingRaw&&(!Number.isFinite(Number(ratingRaw))||Number(ratingRaw)<0||Number(ratingRaw)>5||Number(ratingRaw)*2%1!==0))return alert("평점은 0~5 사이에서 0.5점 단위로 입력해 주세요.");
  if(costRaw&&(!Number.isFinite(Number(costRaw))||Number(costRaw)<0))return alert("비용은 0원 이상으로 입력해 주세요.");
  const id=$("travelPlaceEditId").value,old=(state.travelPlaces||[]).find(x=>x.id===id),before=structuredClone(state.travelPlaces||[]);
  const place=normalizeTravelPlace({id:id||uid(),type:$("travelPlaceType").value,name,visitDate:$("travelPlaceDate").value,location:$("travelPlaceLocation").value,rating:ratingRaw,cost:costRaw,highlight:$("travelPlaceHighlight").value,priceRange:$("travelPlacePriceRange").value,tripId:$("travelPlaceTrip").value,sourceStopId:old?.sourceStopId||"",returnVisit:$("travelPlaceReturn").value,review:$("travelPlaceReview").value,createdAt:old?.createdAt,updatedAt:new Date().toISOString()});
  if(old)Object.assign(old,place);else state.travelPlaces.push(place);
  if(!commit(old?"장소 기록을 수정했습니다.":"장소를 등록했습니다.")){state.travelPlaces=before;renderAll();return}
  closeModal("travelPlaceModal");
};
$("travelStopAdd").onclick=()=>openTravelStopModal();
$("travelStopSave").onclick=()=>{const t=state.travelTrips.find(x=>x.id===$("travelDetailTripId").value);if(!t)return;const id=$("travelStopEditId").value,old=t.itinerary.find(x=>x.id===id),category=$("travelStopCategory").value,row=normalizeTravelStop({id:id||uid(),day:$("travelStopDay").value,date:$("travelStopDate").value,time:$("travelStopTime").value,category,schedule:category,place:$("travelStopPlace").value,note:$("travelStopNote").value,costText:$("travelStopCost").value});if(!row.time&&!row.category&&!row.place&&!row.note&&!row.costText)return alert("시간, 구분, 장소, 비고 또는 예상 비용 중 하나는 입력해 주세요.");const idx=t.itinerary.findIndex(x=>x.id===row.id);if(idx>=0)t.itinerary[idx]=row;else t.itinerary.push(row);t.updatedAt=new Date().toISOString();closeModal("travelStopModal");commit(old?"여행 일정 행을 수정했습니다.":"여행 일정 행을 추가했습니다.",false);openTravelTripDetail(t.id)};

if($("wishlistAdd"))$("wishlistAdd").onclick=()=>openWishlistModal();
if($("wishlistFilter"))$("wishlistFilter").onchange=()=>{uiResetPage('wishlist');state.ui.wishlistFilter=$("wishlistFilter").value;save();renderWishlist()};
if($("wishlistKindFilter"))$("wishlistKindFilter").onchange=()=>{uiResetPage('wishlist');state.ui.wishlistKindFilter=$("wishlistKindFilter").value;save();renderWishlist()};
if($("wishlistSearch"))$("wishlistSearch").oninput=()=>{uiResetPage('wishlist');state.ui.wishlistSearch=$("wishlistSearch").value;renderWishlist()};
...
$("travelItineraryImport").onclick=()=>{const t=state.travelTrips.find(x=>x.id===$("travelDetailTripId").value);if(!t)return;const selected=readTravelImportPreview();if(!selected.length)return alert("저장할 미리보기 일정이 없습니다.");if(selected.some(x=>x.link&&!x.stop.place))return alert("장소 목록에 연결할 행에는 장소 이름을 입력해 주세요.");const beforeTrips=structuredClone(state.travelTrips),beforePlaces=structuredClone(state.travelPlaces);const added=selected.map(x=>x.stop);t.itinerary.push(...added);let linked=0;selected.forEach(({stop,link})=>{if(!link||travelPlaceExistsForStop(t.id,stop))return;state.travelPlaces.push(normalizeTravelPlace({type:travelPlaceTypeFromStop(stop.category),name:stop.place,visitDate:/^\d{4}-\d{2}-\d{2}$/.test(stop.date)?stop.date:t.endDate||t.startDate||"",location:t.destination,highlight:stop.note,priceRange:stop.costText,tripId:t.id,sourceStopId:stop.id,rating:0,review:""}));linked++});t.updatedAt=new Date().toISOString();if(!commit(`여행 일정 ${added.length}개와 방문 장소 ${linked}개를 추가했습니다.`,false)){state.travelTrips=beforeTrips;state.travelPlaces=beforePlaces;renderAll();return}$("travelItineraryBulk").value="";travelImportPreviewRows=[];openTravelTripDetail(t.id)};
...
$("travelDetailDeleteTrip").onclick=()=>{const id=$("travelDetailTripId").value,t=state.travelTrips.find(x=>x.id===id);if(!t||!confirm(`'${t.name}' 여행 기록을 삭제할까요?\n기존 여행 내부의 일정·후기는 함께 삭제됩니다. 독립 장소 기록은 남으며 여행 연결만 풀립니다. 삭제 전 자동 백업을 생성합니다.`))return;exportData({suffix:"before_delete_travel_trip",silent:true});const beforeTrips=state.travelTrips,beforePlaces=structuredClone(state.travelPlaces);state.travelTrips=beforeTrips.filter(x=>x.id!==id);state.travelPlaces.forEach(p=>{if(p.tripId===id)p.tripId=""});if(!commit("여행 기록을 삭제했습니다.")){state.travelTrips=beforeTrips;state.travelPlaces=beforePlaces;renderAll();return}closeModal("travelTripDetailModal")};
