...
- 사용자 화면 또는 배포 파일이 변경되는 작업은 같은 branch에서 `hani-development-history-v1.js`의 공용 개발 히스토리를 함께 갱신한다.
- 메인 화면의 최근 개발 업데이트와 개발센터 전체 히스토리는 반드시 동일한 공용 데이터 원본을 사용한다. 두 화면에 내용을 따로 하드코딩하지 않는다.
- 각 기록에는 날짜, 표시 버전 또는 작업명, 간단한 사용자 관점 요약, 실제 commit 식별자, 정확한 상태를 포함한다.
- 대표 승인 및 Production read-back 전 작업은 `Preview`, 운영 반영과 read-back이 끝난 작업만 `운영 반영`으로 표시한다.
- 문서·개발 절차만 변경한 작업은 사용자에게 의미가 있을 때만 `Process`로 기록한다.
- Release QA에서 메인 최근 기록, 개발센터 전체 목록, 상태 필터, 모바일 표시와 최신 항목 누락 여부를 함께 확인한다.
