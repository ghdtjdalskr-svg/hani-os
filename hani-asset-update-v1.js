...
function holdingEvidence(h){return [h.quantity,h.buyPrice,h.currentPrice,h.purchaseAmount,h.evaluationAmount,h.pnl,h.returnRate].filter(v=>v!==null&&v!==undefined).length}
function holdingIdentity(h){const ticker=text(h.ticker).toUpperCase().replace(/^A(?=\d{6}$)/,""),name=text(h.name).toLocaleLowerCase("ko-KR").replace(/\s+/g,"");return name?`N:${name}`:ticker?`T:${ticker}`:""}
function sameHolding(a,b){const aTicker=text(a.ticker).toUpperCase().replace(/^A(?=\d{6}$)/,""),bTicker=text(b.ticker).toUpperCase().replace(/^A(?=\d{6}$)/,"");if(aTicker&&bTicker&&aTicker!==bTicker)return false;return (!!holdingIdentity(a)&&holdingIdentity(a)===holdingIdentity(b))||!!aTicker&&aTicker===bTicker}
const holdingFields=["instrumentId","name","market","ticker","quantity","buyPrice","currentPrice","purchaseAmount","evaluationAmount","pnl","returnRate","weight","fee","tax"];
function mergeHoldings(rows=[]){const merged=[];for(const raw of rows){const next=makeHolding(raw);if(!holdingIdentity(next)||holdingEvidence(next)===0)continue;const index=merged.findIndex(old=>sameHolding(old,next));if(index<0){merged.push(next);continue}const old=merged[index],primary=holdingEvidence(next)>holdingEvidence(old)?next:old,secondary=primary===next?old:next;for(const k of holdingFields)if((primary[k]===null||primary[k]===undefined||primary[k]==="")&&secondary[k]!==null&&secondary[k]!==undefined&&secondary[k]!=="")primary[k]=secondary[k];merged[index]=primary}return merged}
function mergeWithExistingHoldings(incoming=[],existing=[]){const merged=existing.map(h=>({...h}));for(const next of mergeHoldings(incoming)){const index=merged.findIndex(old=>sameHolding(old,next));if(index<0){merged.push(next);continue}const old=merged[index],updated={...old};for(const k of holdingFields)if(next[k]!==null&&next[k]!==undefined&&next[k]!=="")updated[k]=next[k];merged[index]=updated}return merged}
function mergeScreens(rows=[]){const out={broker:"",accountName:"",accountType:"",accountNumber:"",currentAsset:null,balance:null,assets:null,purchase:null,evaluation:null,pnl:null,returnRate:null,date:"",holdings:[],extractedText:"",screenCount:rows.length};for(const row of rows){for(const k of ["broker","accountName","accountType","accountNumber","date"])if(!out[k]&&row[k])out[k]=row[k];for(const k of ["currentAsset","balance","purchase","evaluation","pnl","returnRate"])if(out[k]===null&&row[k]!==null)out[k]=row[k];out.holdings.push(...(row.holdings||[]));out.extractedText=[out.extractedText,row.extractedText].filter(Boolean).join("\n")}
  out.holdings=mergeHoldings(out.holdings);
...
window.HANI_ASSET_UPDATE_V1={normalize,validation,sameAccount,merge,mergeScreens,mergeHoldings,mergeWithExistingHoldings,resolveAccountMatch,typeMatch,accountDeleteImpact,missingReviewFields};
if(!root)return;
function isCash(t){return /입출금|현금|저축|파킹|CMA|예금|적금|주택|비상금/i.test(text(t))}
function missingReviewFields(d,account,old){
  const fields=[],cash=isCash(account?.type),known=v=>v!==null&&v!==undefined&&v!=="";
  const accountField=(key,label,required,previous)=>{if(!known(d[key])&&!known(previous))fields.push({scope:"account",key,label,required})};
  if(!d.date)fields.push({scope:"account",key:"date",label:"기준일",required:true,type:"date"});
  if(!known(d.currentAsset)&&!(cash&&known(d.balance))&&!(known(d.evaluation)&&known(d.balance)))fields.push({scope:"account",key:"currentAsset",label:"계좌 총액",required:true});
  if(!cash){
    accountField("balance","예수금",false,null);
    accountField("purchase","주식 매입금액",false,old?.totalPurchase);
    accountField("evaluation","주식 평가금액",!known(old?.totalEvaluation),old?.totalEvaluation);
    accountField("pnl","평가손익",false,old?.totalPnl);
    accountField("returnRate","수익률",false,old?.totalReturn);
    (d.holdings||[]).forEach((raw,index)=>{const holding=makeHolding(raw),previous=(old?.holdings||[]).find(saved=>sameHolding(saved,holding));for(const [key,label,required] of [["quantity","수량",true],["buyPrice","평균단가",true],["currentPrice","현재가",true],["evaluationAmount","평가금액",true],["pnl","평가손익",false],["returnRate","수익률",false]])if(!known(holding[key])&&!known(previous?.[key]))fields.push({scope:"holding",index,key,label:`${holding.name||holding.ticker||`종목 ${index+1}`} · ${label}`,required});});
  }
  return fields;
}
function setReviewAnswer(field,raw){
  if(!draft)return false;
  const answer=field.type==="date"?dateValue(raw):number(raw);
  if(answer===null||answer===""){saveMessage(`${field.label} 값을 확인해 입력해 주세요. 저장 전까지 변경되지 않습니다.`,"error");return false}
  if(field.type!=="date"&&!["pnl","returnRate"].includes(field.key)&&answer<0){saveMessage(`${field.label}은(는) 음수일 수 없습니다.`,"error");return false}
  if(field.scope==="account"){
    draft[field.key]=answer;
    if(field.key==="currentAsset")draft.assets=answer;
  }else if(draft.holdings?.[field.index])draft.holdings[field.index][field.key]=answer;
  if(draft.currentAsset!==null&&draft.evaluation!==null&&draft.balance!==null)draft.reconciliation={difference:Math.abs(Math.round(draft.currentAsset-draft.evaluation-draft.balance)),total:draft.currentAsset,evaluation:draft.evaluation,cash:draft.balance};
  else draft.reconciliation=null;
  render();saveMessage(`${field.label} 확인값이 Preview에 반영됐습니다. 승인 전에는 저장되지 않습니다.`,"success");return true;
}
...
function render(){
  ...
  const match=resolution.account,v=validation(draft),old=draft.original||(match&&latestFor(match.id).account),imageMode=draft.mode==="image",reviewFields=imageMode?missingReviewFields(draft,match,old):[],requiredReview=reviewFields.filter(field=>field.required);
  ...
  const note=imageMode&&match?`<p class="asset-account-hint"><b>선택 계좌 우선:</b> OCR이 다른 계좌명·상품명을 읽어도 ${safe(match.name)} accountId에만 반영합니다.</p>`:imageMode?`<p class="asset-account-hint">업데이트할 기존 계좌를 먼저 선택해 주세요.</p>`:"";
  ...
  const reviewPanel=reviewFields.length?`<section class="asset-review-panel" aria-label="읽지 못한 값 확인"><div class="asset-review-head"><b>화면에서 읽지 못한 값 확인</b><span>${requiredReview.length?`필수 ${requiredReview.length}개 확인 후 저장 가능`:"선택 항목은 비워두면 기존값 유지"}</span></div><p>보이는 숫자만 직접 입력해 주세요. OCR 값을 추정해 채우지 않으며 입력 중에는 저장되지 않습니다.</p><div class="asset-review-grid">${reviewFields.map((field,index)=>`<label class="field"><span>${safe(field.label)} ${field.required?'<em>필수</em>':'<small>선택</small>'}</span><input data-asset-review="${index}" type="${field.type==="date"?"date":"number"}" ${field.type==="date"?"":`step="any" ${["pnl","returnRate"].includes(field.key)?"":"min=\"0\""}`} placeholder="확인한 값 입력"></label>`).join("")}</div></section>`:"";
  const holdingPreview=draft.holdings?.length?`<div class="asset-holding-preview"><b>인식한 보유종목 · 동일 종목 병합 후 ${draft.holdings.length}개</b><div class="tw"><table><thead><tr><th>종목</th><th>수량</th><th>평균단가</th><th>현재가</th><th>평가금액</th><th>평가손익</th><th>수익률</th></tr></thead><tbody>${draft.holdings.map(h=>{const x=makeHolding(h),previous=(old?.holdings||[]).find(saved=>sameHolding(saved,x)),shown=mergeWithExistingHoldings([x],previous?[previous]:[])[0]||x;return `<tr><td>${safe(shown.name||shown.ticker||"미확인")}</td><td>${shown.quantity===null?"미확인":safe(shown.quantity)}</td><td>${shown.buyPrice===null?"미확인":money(shown.buyPrice)}</td><td>${shown.currentPrice===null?"미확인":money(shown.currentPrice)}</td><td>${shown.evaluationAmount===null?"미확인":money(shown.evaluationAmount)}</td><td>${shown.pnl===null?"미확인":money(shown.pnl)}</td><td>${shown.returnRate===null?"미확인":safe(shown.returnRate+"%")}</td></tr>`}).join("")}</tbody></table></div><small>미확인 값은 추정하지 않습니다. 기존 저장값이 있으면 유지하며, 위 표는 저장 후 적용될 값을 표시합니다.</small></div>`:"";
  preview.innerHTML=`<article class="asset-capture-preview"><header><b>${safe(match?.name||(imageMode?"계좌 확인 필요":accountLabel(draft)))}</b><span>${headline}</span></header><div class="asset-preview-grid">${fields.filter(([,value])=>value!==null&&value!==undefined&&value!==""&&value!=="미확인").map(([key,value])=>`<div class="asset-preview-field"><span>${safe(key)}</span><b>${safe(value)}</b></div>`).join("")}</div>${holdingPreview}${reviewPanel}<div class="asset-validation ${v.severity.toLowerCase()}" role="status"><b>${safe(v.severity)}</b><span>${safe(v.message)}</span></div>${reconciliation}${missingPreserved}${note}<div class="asset-preview-actions"><button class="btn" id="assetPreviewCancel" type="button">취소</button><button class="btn finance" id="assetPreviewApprove" type="button" ${v.blocking||draft.reconciliation?.difference>100||!draft.date||requiredReview.length||resolution.kind==="missing"||(imageMode&&!match)?"disabled":""}>승인하고 저장</button></div></article>`;
document.getElementById("assetPreviewCancel").onclick=()=>{draft=null;render()};
preview.querySelectorAll("[data-asset-review]").forEach(input=>input.onchange=()=>{const field=reviewFields[Number(input.dataset.assetReview)],answer=input.value;setTimeout(()=>setReviewAnswer(field,answer),0)});
document.getElementById("assetPreviewApprove").onclick=approve}
function makeHolding(h){const raw={id:h?.id,instrumentId:text(value(h,["instrumentId","instrument_id"])),name:text(value(h,["name","symbol_name","종목명"])),market:text(value(h,["market","시장"])),ticker:text(value(h,["ticker","symbol","종목코드"])),quantity:number(value(h,["quantity","qty","수량"])),buyPrice:number(value(h,["buyPrice","averagePrice","average_price","avg_price","평균단가"])),currentPrice:number(value(h,["currentPrice","current_price","현재가"])),purchaseAmount:number(value(h,["purchaseAmount","purchase_amount","매입금액"])),evaluationAmount:number(value(h,["evaluationAmount","valuationAmount","evaluation_amount","평가금액"])),pnl:number(value(h,["profitLoss","pnl","평가손익"])),returnRate:number(value(h,["returnRate","return_rate","수익률"])),weight:number(value(h,["weight","비중"])),fee:number(value(h,["fee","수수료"])),tax:number(value(h,["tax","세금"]))};return typeof normalizeBrokerHolding==="function"?normalizeBrokerHolding(raw):raw}
...
function approve(){
  ...
  if(draft.mode==="image"){const required=missingReviewFields(draft,resolution.account,latestFor(draft.accountId).account).filter(field=>field.required);if(required.length)return saveMessage(`저장 실패: ${required.map(field=>field.label).join("·")} 확인이 필요합니다.`,"error")}
...
async function run(){if(!selected.length)return;const target=selectedAccount();if(!target)return saveMessage("분석할 기존 계좌를 먼저 선택해 주세요.","error");saveMessage(`${target.name} 계좌 화면을 분석하고 있어요.`);analyze.disabled=true;try{const rows=[];for(const file of selected){const image_data_url=await intakeImageToDataUrl(file),result=await agentApi("extract_intake_image",{image_data_url,target_hint:"asset",file_name:file.name,account_context:{accountId:target.id,accountName:target.name,broker:target.broker,accountType:target.type}}),extraction=agentObj(result.extraction),next=normalize(extraction);if(extraction.target_hint!=="asset")throw new Error("Asset Vision 계약을 확인할 수 없습니다.");if(next.assets===null&&!next.accountName&&!next.broker&&!next.holdings.length)throw new Error(`${file.name}: 계좌 값을 구조화해 읽지 못했습니다.`);if(rows.length&&rows[0].accountNumber&&next.accountNumber&&rows[0].accountNumber!==next.accountNumber)throw new Error("서로 다른 계좌번호가 보이는 이미지가 섞여 있습니다. 계좌별로 나눠 올려주세요.");rows.push(next)}const combined=mergeScreens(rows);draft={...combined,accountId:target.id,selectedByUser:true,mode:"image"};render();const screenNote=rows.length>1?`${rows.length}개 화면 합산 · `:"";saveMessage(`${screenNote}${target.name} 계좌 지정 완료 · 읽지 못한 값은 Preview에서 확인해 주세요.`,"success")}catch(e){saveMessage(e?.message||String(e),"error")}finally{updateCaptureReadyState()}}
...
function renderSelectedFiles(){list.textContent=selected.length?selected.map((f,i)=>`${i+1}. ${f.name||"계좌 스크린샷"}`).join(" · "):"추가된 계좌 화면이 없습니다.";updateCaptureReadyState();draft=null;render()}
...
function handleAssetPaste(event){if(!selectedAccount())return saveMessage("붙여넣기 전에 업데이트할 기존 계좌를 먼저 선택해 주세요.","error");const items=[...(event.clipboardData?.items||[])],raw=items.filter(item=>item.kind==="file"&&/^image\//i.test(item.type||"")).map(item=>item.getAsFile()).filter(Boolean),images=raw.length?raw:[...(event.clipboardData?.files||[])].filter(file=>/^image\//i.test(file.type||""));if(!images.length)return;event.preventDefault();const pasted=images.map(clipboardImage);if(pasted.some(file=>!file))return saveMessage("붙여넣기 실패: PNG·JPG·WEBP 이미지만 사용할 수 있습니다.","error");if(pasted.some(file=>file.size>12*1024*1024))return saveMessage("붙여넣기 실패: 원본 이미지는 장당 12MB 이하만 사용할 수 있습니다.","error");selected=[...selected,...pasted];renderSelectedFiles();saveMessage(`클립보드 이미지 ${pasted.length}장 첨부 · ${selectedAccount().name} 계좌 분석 준비 완료 · 저장 전 자산 데이터 변경 없음.`,"success")}
files.onchange=()=>{if(!selectedAccount()){files.value="";return saveMessage("스크린샷을 추가하기 전에 기존 계좌를 먼저 선택해 주세요.","error")}selected=[...files.files];renderSelectedFiles()};
if(accountSelect)accountSelect.onchange=()=>{selectedAccountId=accountSelect.value;draft=null;render();updateCaptureReadyState();saveMessage(selectedAccount()?`${selectedAccount().name} 계좌를 선택했습니다. 이제 스크린샷을 추가해 주세요.`:"업데이트할 기존 계좌를 먼저 선택해 주세요.",selectedAccount()?"success":"info")};
if(pasteZone){pasteZone.onclick=()=>{if(!selectedAccount())return saveMessage("붙여넣기 전에 업데이트할 기존 계좌를 먼저 선택해 주세요.","error");pasteZone.focus()};pasteZone.onpaste=handleAssetPaste}
analyze.onclick=run;
