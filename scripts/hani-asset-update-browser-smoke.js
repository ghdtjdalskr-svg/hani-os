...
(async()=>{
  ...
  await page.evaluate(key=>{const s=JSON.parse(localStorage.getItem(key));s.accounts.push({id:"wrong-ocr",name:"홍성민",type:"위탁종합",broker:"하나증권",number:"",openingCash:0});s.investmentBrokerSnapshots[0].accounts.push({id:"snapshot-account-wrong",accountId:"wrong-ocr",accountName:"홍성민",enabled:true,estimatedAssets:160490,totalPurchase:null,totalEvaluation:158220,totalPnl:null,totalReturn:null,loanAmount:null,holdings:[]});s.ui.series.push("wrong-ocr");localStorage.setItem(key,JSON.stringify(s));},storageKey);
  await page.reload({waitUntil:"networkidle"});
  await page.evaluate(()=>{unlockLoginGate();cloudClient={auth:{getSession:async()=>({data:{session:{access_token:"browser-smoke"}},error:null})}};cloudUser={id:"browser-smoke"}});
  await page.getByRole("button",{name:"자산 업데이트",exact:true}).click();
